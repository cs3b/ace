...
RSpec.describe CodingAgentTools::Molecules::HTTPRequestBuilder do
  ...
  describe "private methods" do
    describe "#build_url_with_query" do
      it "returns URL unchanged when no query params" do
        url = builder.send(:build_url_with_query, "https://example.com/path", nil)
        expect(url).to eq("https://example.com/path")

        url = builder.send(:build_url_with_query, "https://example.com/path", {})
        expect(url).to eq("https://example.com/path")
      end
      ...
      it "merges with existing query parameters" do
        ...
        expect(params).to include("existing" => "value", "new" => "param")
      end
