...
RSpec.describe CodingAgentTools::Cli::Commands::TogetherAI::Query do
  let(:command) { described_class.new }
  let(:api_key) { "test-api-key" }
  let(:prompt) { "Test prompt" }
  let(:mock_client) { instance_double(CodingAgentTools::Organisms::TogetherAIClient) }
  ...
  before do
    ...
    allow(ENV).to receive(:[]).with("TOGETHER_API_KEY").and_return(api_key)
  ...
  describe "#call" do
    context "with timeout parameter" do
      it "passes timeout to TogetherAIClient" do
        expect(CodingAgentTools::Organisms::TogetherAIClient).to receive(:new).with(
    ...
    context "with valid prompt" do
      it "queries Together AI and outputs response" do
    ...
    context "with custom model" do
      it "passes model to client" do
        expect(CodingAgentTools::Organisms::TogetherAIClient).to receive(:new).with(
          hash_including(model: "mistralai/Mistral-8x7B-Instruct-v0.1")
        ).and_return(mock_client)

        command.call(prompt: prompt, model: "mistralai/Mistral-8x7B-Instruct-v0.1")
      end
    end
    ...
    context "with error handling" do
      it "handles errors gracefully without debug" do
        ...
        }.to output(/Error: Failed to query Together AI: API Error/).to_stderr
      end
      ...
      it "handles errors with debug information" do
        ...
        }.to output(/Error: CodingAgentTools::Error: Failed to query Together AI: API Error/).to_stderr
      end
    end
  ...
  describe "command metadata" do
    it "has correct description" do
      expect(described_class.new.description).to eq("Query Together AI API with a prompt")
    end
