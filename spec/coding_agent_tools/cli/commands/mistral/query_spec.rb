...
RSpec.describe CodingAgentTools::Cli::Commands::Mistral::Query do
  let(:command) { described_class.new }
  let(:api_key) { "test-api-key" }
  let(:prompt) { "Test prompt" }
  let(:mock_client) { instance_double(CodingAgentTools::Organisms::MistralClient) }
  ...
  before do
    ...
    allow(ENV).to receive(:[]).with("MISTRAL_API_KEY").and_return(api_key)
  ...
  describe "#call" do
    context "with timeout parameter" do
      it "passes timeout to MistralClient" do
        expect(CodingAgentTools::Organisms::MistralClient).to receive(:new).with(
    ...
    context "with valid prompt" do
      it "queries Mistral and outputs response" do
    ...
    context "with custom model" do
      it "passes model to client" do
        expect(CodingAgentTools::Organisms::MistralClient).to receive(:new).with(
          hash_including(model: "mistral-medium-latest")
        ).and_return(mock_client)

        command.call(prompt: prompt, model: "mistral-medium-latest")
      end
    end
    ...
    context "with error handling" do
      it "handles errors gracefully without debug" do
        ...
        }.to output(/Error: Failed to query Mistral AI: API Error/).to_stderr
      end
      ...
      it "handles errors with debug information" do
        ...
        }.to output(/Error: CodingAgentTools::Error: Failed to query Mistral AI: API Error/).to_stderr
      end
    end
  ...
  describe "command metadata" do
    it "has correct description" do
      expect(described_class.new.description).to eq("Query Mistral AI API with a prompt")
    end
