...
RSpec.describe CodingAgentTools::Atoms::DirectoryScanner do
  ...
  describe ".scan_files" do
    context "with basic directory scanning" do
      before do
        ...
        File.chmod(0755, File.join(temp_dir, "test2.rb"))
        File.chmod(0755, File.join(temp_dir, "script.sh"))
      end
    ...
    context "with subdirectories" do
      before do
        ...
        File.chmod(0755, File.join(subdir, "nested.sh"))
      end
    ...
    context "with error handling" do
      it "raises ArgumentError for non-existent directory" do
        ...
        }.to raise_error(ArgumentError, "Directory does not exist: #{non_existent}")
      end
      ...
      it "raises ArgumentError for file path instead of directory" do
        ...
        }.to raise_error(ArgumentError, "Directory does not exist: #{file_path}")
      end
    end
    ...
    context "with files of different permissions" do
      before do
        ...
        File.chmod(0644, File.join(temp_dir, "readable.txt"))
        File.chmod(0755, File.join(temp_dir, "executable.sh"))
      end
  ...
  describe ".scan_with_info" do
    before do
      ...
      File.chmod(0755, File.join(temp_dir, "small.sh"))
      File.chmod(0755, File.join(temp_dir, "large.rb"))
