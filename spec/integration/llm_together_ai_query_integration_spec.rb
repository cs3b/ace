...
RSpec.describe "llm-together-ai-query integration", type: :integration do
  include ProcessHelpers

  let(:exe_name) { "llm-together-ai-query" }
  ...
  describe "command execution" do
    it "shows help when requested" do
      ...
      expect(stdout).to match(/Query Together AI API with a prompt/)
  ...
  describe "API integration" do
    context "with valid API key" do
      it "queries Together AI with a simple prompt", :vcr do
        cassette_name = "llm_together_ai_query_integration/queries_together_ai_with_simple_prompt"
      ...
      it "outputs JSON format when requested", :vcr do
        cassette_name = "llm_together_ai_query_integration/outputs_json_format"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)
      ...
      it "reads prompt from file", :vcr do
        cassette_name = "llm_together_ai_query_integration/reads_prompt_from_file"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)
      ...
      it "supports custom model selection", :vcr do
        cassette_name = "llm_together_ai_query_integration/supports_custom_model"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)

        stdout, stderr, status = execute_gem_executable(exe_name,
          ["Count from 1 to 3", "--model", "meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo"], env: env)
      ...
      it "supports custom temperature", :vcr do
        cassette_name = "llm_together_ai_query_integration/supports_custom_temperature"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)
      ...
      it "supports output to file", :vcr do
        cassette_name = "llm_together_ai_query_integration/supports_output_to_file"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)
      ...
      it "supports system instruction", :vcr do
        cassette_name = "llm_together_ai_query_integration/supports_system_instruction"
        env = vcr_subprocess_env(cassette_name, "TOGETHER_API_KEY" => api_key)
