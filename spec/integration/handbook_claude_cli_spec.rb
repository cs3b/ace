...
RSpec.describe "handbook claude commands" do
  ...
  describe "handbook --help" do
    it "displays help including claude commands" do
      ...
      expect(output).to include("claude-generate-commands")
      expect(output).to include("claude-update-registry")
      expect(output).to include("claude-integrate")
      expect(output).to include("claude-validate")
      expect(output).to include("claude-list")
    end
  end

  # Removed - no claude namespace with hyphenated approach
  ...
  describe "handbook claude-generate-commands" do
    it "displays not implemented message" do
      output, status = Open3.capture2e("bundle", "exec", exe_path, "claude-generate-commands")
      
      expect(status).to be_success
      expect(output).to include("generate-commands: Not yet implemented")
      expect(output).to include("This will scan workflow files")
    end
  end
