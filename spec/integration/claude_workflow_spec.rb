...
RSpec.describe "Claude Integration Workflow" do
  ...
  describe "CLI integration" do
    ...
    it "shows help for all Claude subcommands" do
      ["generate-commands", "update-registry", "integrate", "validate", "list"].each do |subcommand|
        stdout, _, status = execute_command(["bundle", "exec", "handbook", "claude", subcommand, "--help"])
  ...
  describe "command functionality" do
    ...
    it "runs generate-commands with dry-run" do
      stdout, _, status = execute_command(["bundle", "exec", "handbook", "claude", "generate-commands", "--dry-run"])
      expect(status).to be_success
      expect(stdout).to include("Scanning workflow instructions")
    end

    it "runs update-registry command" do
      stdout, _, status = execute_command(["bundle", "exec", "handbook", "claude", "update-registry"])
      expect(status).to be_success
      expect(stdout).to include("Not yet implemented")
    end

    it "runs validate command" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "validate"])
    ...
    it "runs list command" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "list"])
      expect(status).to be_success
    end

    it "runs integrate with dry-run" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "integrate", "--dry-run"])
      expect(status).to be_success
  ...
  describe "command options" do
    it "accepts force option for generate-commands" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "generate-commands", "--force", "--dry-run"])
      expect(status).to be_success
    end

    it "accepts format option for list" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "list", "--format", "json"])
      expect(status).to be_success
    end

    it "accepts type option for list" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "list", "--type", "custom"])
      expect(status).to be_success
    end

    it "accepts workflow option for generate-commands" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "generate-commands", "--workflow", "test", "--dry-run"])
      expect(status).to be_success
    end
  end
  ...
  describe "error handling" do
    it "shows error for unknown subcommand" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "unknown-command"])
      expect(status.exitstatus).to eq(1)
    end

    it "shows error for invalid option" do
      _, _, status = execute_command(["bundle", "exec", "handbook", "claude", "list", "--invalid-option"])
      expect(status.exitstatus).to eq(1)
    end
  end
end
