...
RSpec.describe "llm-mistral-query integration", type: :integration do
  include ProcessHelpers

  let(:exe_name) { "llm-mistral-query" }
  ...
  describe "command execution" do
    it "shows help when requested" do
      ...
      expect(stdout).to match(/Query Mistral AI API with a prompt/)
    ...
    it "requires a prompt argument" do
      ...
      expect(stderr).to match(/ERROR: "llm-mistral-query" was called with no arguments/)
    end
  end
  ...
  describe "API integration" do
    context "with valid API key" do
      it "queries Mistral with a simple prompt", :vcr do
        cassette_name = "llm_mistral_query_integration/queries_mistral_with_simple_prompt"
      ...
      it "outputs JSON format when requested", :vcr do
        cassette_name = "llm_mistral_query_integration/outputs_json_format"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)
      ...
      it "reads prompt from file", :vcr do
        cassette_name = "llm_mistral_query_integration/reads_prompt_from_file"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)
      ...
      it "supports custom model selection", :vcr do
        cassette_name = "llm_mistral_query_integration/supports_custom_model"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)

        stdout, stderr, status = execute_gem_executable(exe_name,
          ["Count from 1 to 3", "--model", "mistral-small-latest"], env: env)
      ...
      it "supports custom temperature", :vcr do
        cassette_name = "llm_mistral_query_integration/supports_custom_temperature"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)
      ...
      it "supports output to file", :vcr do
        cassette_name = "llm_mistral_query_integration/supports_output_to_file"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)
      ...
      it "supports system instruction", :vcr do
        cassette_name = "llm_mistral_query_integration/supports_system_instruction"
        env = vcr_subprocess_env(cassette_name, "MISTRAL_API_KEY" => api_key)
    ...
    context "with invalid API key" do
      it "shows authentication error" do
        env = vcr_subprocess_env("llm_mistral_query_integration/invalid_api_key", "MISTRAL_API_KEY" => "invalid-key")
