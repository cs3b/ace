...
```bash
ls -la .ace/tools/exe/ | head -10 | sed 's/^/    /'
```
...
- Audit all executables in `.ace/tools/exe/` to identify inconsistent implementation patterns
...
- .ace/tools/exe/git-commit (refactor to use ExecutableWrapper)
- .ace/tools/exe/git-status (refactor to use ExecutableWrapper)
- .ace/tools/exe/code-lint (refactor to use ExecutableWrapper)
...
- [x] Audit all executables in .ace/tools/exe/ and categorize by implementation pattern
  > TEST: Pattern Analysis
  > Type: Pre-condition Check
  > Assert: All executables categorized as either ExecutableWrapper or manual parsing
  > Command: find .ace/tools/exe/ -type f -exec head -15 {} \; | grep -E "(ExecutableWrapper|ARGV|case.*when)"
- [x] Analyze ExecutableWrapper molecule to understand its interface and requirements
  ...
  > Command: cd .ace/tools && rspec spec/unit/coding_agent_tools/molecules/executable_wrapper_spec.rb -v
...
- [x] Refactor git-commit executable to use ExecutableWrapper pattern
  ...
  > Command: cd .ace/tools && exe/git-commit --help && echo "Test intention flag:" && exe/git-commit --intention "test" --dry-run
- [x] Refactor git-status executable to use ExecutableWrapper pattern
  ...
  > Command: cd .ace/tools && exe/git-status --help && exe/git-status --verbose
- [x] Refactor code-lint executable to use ExecutableWrapper pattern (if it uses manual parsing)
  ...
  > Command: cd .ace/tools && exe/code-lint --help
...
- [x] Verify all executables use consistent implementation pattern
  ...
  > Assert: All executables in .ace/tools/exe/ use ExecutableWrapper pattern
  > Command: find .ace/tools/exe/ -type f -exec grep -l "ExecutableWrapper" {} \; | wc -l && find .ace/tools/exe/ -type f | wc -l

## Acceptance Criteria

- [x] All executables in .ace/tools/exe/ use the ExecutableWrapper molecule pattern
...
- Good example: .ace/tools/exe/llm-query (uses ExecutableWrapper)
- Good example: .ace/tools/exe/git-add (uses ExecutableWrapper)
- Needs refactoring: .ace/tools/exe/git-commit (manual parsing)
- Needs refactoring: .ace/tools/exe/git-status (manual parsing)
