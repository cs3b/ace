...
```bash
ls -1 bin/
ls -1 coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/_binstubs/ # Check for relevant binstubs
...
coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/_binstubs/
build
lint
run
test
  ...
  - `bin/gc`: Wrapper for `git commit`. This is the most complex script and needs to:
    ...
    - **LLM Interaction:**
      - Provide a mechanism to call LLMs for commit message generation. This will likely involve creating two helper
        scripts in `.ace/tools/exe-old/` (e.g., `.ace/tools/exe-old/llm_query_gemini.sh` and
        `.ace/tools/exe-old/llm_query_lms.sh`) that encapsulate the `curl` calls and API key
...
- `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_gemini.sh` (executable helper script for Gemini API
  calls)
- `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_lms.sh` (executable helper script for LM Studio API
  calls)
- `coding-agent-workflow-toolkit-meta/.ace/handbook/prompts/gc_llm_commits.guide.md` (copy or adapt from
...
2. **Implementation (Helper LLM Scripts):**
    - Create `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_gemini.sh`.
    - Create `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_lms.sh`.
    - Create/copy `coding-agent-workflow-toolkit-meta/.ace/handbook/prompts/gc_llm_commits.guide.md`.
...
- [ ] **Phase 2: Implementation (Helper LLM Scripts & Guide)**
  - [ ] Create `coding-agent-workflow-toolkit-meta/.ace/handbook/prompts/gc_llm_commits.guide.md` by copying content
  ...
  - [ ] Implement `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_gemini.sh`:
    - [ ] Add shebang `#!/bin/sh`.
    ...
    - [ ] `chmod +x`.
  - [ ] Implement `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_lms.sh`:
    - [ ] Add shebang `#!/bin/sh`.
...
- [ ] **Phase 4: Implementation (`bin/gc`)**
  ...
  - [ ] Read `COMMIT_GUIDE_CONTENT` from
    `coding-agent-workflow-toolkit-meta/.ace/handbook/prompts/gc_llm_commits.guide.md`.
  ...
  - [ ] Conditional call to LLM helper:
    - If `USE_LOCAL_LLM` is true,
      `RAW_COMMIT_MSG=$(./coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_lms.sh -s \"$SYSTEM_PROMPT\" \"$USER_PROMPT\")`.
    - Else,
      `RAW_COMMIT_MSG=$(./coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_gemini.sh -s \"$SYSTEM_PROMPT\" \"$USER_PROMPT\")`.
...
- [ ] `bin/gs`, `bin/gl`, `bin/gp`, `bin/gc` scripts exist in the project's `bin/` directory and are
  executable.
- [ ] `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_gemini.sh` and
  `coding-agent-workflow-toolkit-meta/.ace/tools/exe-old/llm_query_lms.sh` exist, are executable, and can
  successfully query their respective LLMs.
- [ ] `coding-agent-workflow-toolkit-meta/.ace/handbook/prompts/gc_llm_commits.guide.md` exists and
  contains the commit
...
- `coding-agent-workflow-toolkit-meta/.ace/handbook/guides/write-actionable-task.md`
