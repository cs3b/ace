...
- [x] TaskSlugGenerator molecule created for deterministic slug generation
- [x] path_builder.rb updated to use new naming convention
- [x] task_loader.rb supports both old and new formats (backward compatibility)
- [x] Migration utility created to convert existing tasks
- [x] All tests pass with new naming convention
- [x] ace-nav can discover tasks using descriptive paths

## Planning Steps

* [x] Research current task path generation in path_builder.rb
* [x] Analyze task_loader.rb parsing logic
* [x] Design migration strategy for existing tasks
* [x] Review ace-context PresetManager pattern for reference

## Execution Steps

- [x] Create `ace-taskflow/lib/ace/taskflow/molecules/task_slug_generator.rb`
  - Implement deterministic slug generation from task metadata
  - Add configurable type prefixes
  - Ensure slug length limits

- [x] Update `ace-taskflow/lib/ace/taskflow/atoms/path_builder.rb`
...
- [x] Update `ace-taskflow/lib/ace/taskflow/organisms/task_manager.rb`
  - Use TaskSlugGenerator in `create_task` method
  - Update task directory creation logic

- [x] Create migration utility `ace-taskflow/lib/ace/taskflow/organisms/task_migrator.rb`
