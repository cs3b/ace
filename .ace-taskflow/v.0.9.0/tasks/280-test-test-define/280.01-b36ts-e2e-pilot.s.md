...
- [x] 1 scenario directory with `scenario.yml` (tags, cost-tier, sandbox-layout, e2e-justification)
- [x] 8 standalone runner files (`TC-*.runner.md`) with Goal + Workspace + Constraints (no Steps, no hardcoded commands)
- [x] 8 standalone verify files (`TC-*.verify.md`) with Expectations + Verdict format (artifact-based, no access to runner reasoning)
- [x] `scenario.yml` defines `sandbox-layout` showing `results/` tree structure for all 8 goals
- [x] No bash commands, flag names, or expected outputs hardcoded in any runner or verifier file
- [x] All goals justify E2E value that unit tests cannot provide
- [x] Goals 4–8 restore all lost E2E coverage from the original 7 procedural TCs
