...
- [x] **Document Discovery**: Automatically finds all markdown files with ace-docs frontmatter
- [x] **Freshness Tracking**: Shows last-updated dates and identifies stale documents based on configured frequencies
- [x] **Change Analysis**: Generates relevant change summaries based on each document's declared sources
- [x] **Metadata Management**: Updates frontmatter fields (dates, versions) accurately
- [x] **Rule Validation**: Enforces max-lines, required sections, and no-duplicate rules
- [x] **Context Integration**: Leverages ace-context for project awareness
...
* [x] Research YAML frontmatter parsing approaches in Ruby
* [x] Analyze ace-taskflow's frontmatter handling for patterns
* [x] Study ace-context's preset and discovery mechanisms
* [x] Review git command patterns for change detection
* [x] Design caching strategy for diff analysis results
* [x] Plan integration points with ace-llm-query
* [x] Define document discovery search patterns
* [x] Design validation rule engine architecture

### Execution Steps

- [x] Create ace-docs gem structure
  ```bash
...
- [x] Create change detector molecule
  ```ruby
  # lib/ace/docs/molecules/change_detector.rb
  # Analyzes git history and file changes
  ```

- [x] Implement diff command with LLM integration
  ```ruby
...
- [x] Create frontmatter manager molecule
  ```ruby
...
- [x] Create update-docs workflow instruction
  ```markdown
...
- [x] Create Claude command integration
  ```markdown
