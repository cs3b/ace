...
- [ ] Update execute_simple method to use config timeout
  > Change hardcoded 10 to use Ace::Review.config["defaults"]["gh_timeout"]
  > Or add separate execute_simple_timeout to config

- [ ] Consider if execute_simple needs different timeout than regular execute
  > If yes, add `gh_simple_timeout` to .ace-defaults/review/config.yml

- [ ] Run tests with `ace-test ace-review`

- [ ] Bump patch version

## Acceptance Criteria

- [ ] execute_simple uses configurable timeout
- [ ] All ace-review tests pass
- [ ] Gem version bumped
