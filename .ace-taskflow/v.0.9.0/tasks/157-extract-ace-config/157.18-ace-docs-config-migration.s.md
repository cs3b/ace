...
- [ ] Refactor DocumentRegistry to use `Ace::Config.resolve_namespace("docs")`
  > Remove custom YAML loading logic (lines 101-139)
  > Replace with Ace::Docs.config access pattern

- [ ] Update Document model to get freshness thresholds from config
  > Replace hardcoded values in freshness_status method
  > Use Ace::Docs.config["default_freshness_days"]

- [ ] Run tests with `ace-test ace-docs`

- [ ] Bump patch version in ace-docs/lib/ace/docs/version.rb

## Acceptance Criteria

- [ ] DocumentRegistry uses ace-config for configuration loading
- [ ] Document model reads freshness thresholds from .ace-defaults/docs/config.yml
- [ ] All ace-docs tests pass
- [ ] Gem version bumped