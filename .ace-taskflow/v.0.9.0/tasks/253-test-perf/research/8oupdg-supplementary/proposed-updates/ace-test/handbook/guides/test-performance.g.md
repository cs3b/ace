...
## Zombie Mocks (Fast-Loop Leak)

Zombie mocks are stubs for code paths that no longer run. Tests pass but execute real IO, leading to slow performance. Detect with profiling; fix by aligning stubs to the current code path.

## Composite Helpers

Use composite helpers to reduce nested stubs and ensure caches are pre-warmed consistently.

## Contract Testing for External APIs

When mocks represent external APIs, validate them against real responses or schemas to prevent drift.

## Stub the Outer Boundary

Tests often look stubbed but still trigger subprocess calls because a guard method runs first.
Always stub the **outermost method** that can perform IO (for example `available?`, `system_has_command?`, or a facade method).

**Anti-pattern:**
```ruby
Open3.stub(:capture3, mock_result) do
  Runner.run("file.rb") # Still calls available? -> system()
end
```
...
## Cache Hygiene and Pre-Warm

Random slowness is often cache invalidation or test-order side effects.

**Guidelines:**
- Avoid resetting shared caches in `setup`
- Pre-warm expensive caches once in `test_helper.rb`
- If you must reset, reset and re-populate inside a helper before yielding

## Performance Audit Cadence

- **Per PR**: run `ace-test --profile 10` for packages touched by tests
- **Nightly/weekly**: run the full suite and compare slowest tests
- **Quarterly**: migrate stale integration tests to E2E or mocked unit tests
