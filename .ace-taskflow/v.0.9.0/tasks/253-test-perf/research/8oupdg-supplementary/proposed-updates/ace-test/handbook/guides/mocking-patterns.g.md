...
### Stub Availability or Guard Methods

Guard methods often run before subprocess calls. Stub these at the boundary to prevent hidden IO.
...
If stubs no longer match the real code path, tests may pass while running real IO. Profile slow tests and update stubs to match current boundaries.

### Contract Testing for External APIs

Mocks should match real API behavior. Prefer schema validation or snapshots to prevent drift.

### Stub Availability or Guard Methods

Many runners call `available?` or similar guard methods that perform a subprocess check. If you only stub `Open3.capture3`, the test can still be slow because the guard runs first.
...
### Cache-Safe Stub Helpers

If a class caches availability or configuration, reset and pre-populate inside the helper to avoid test order dependency:

```ruby
def with_stubbed_runner(available:)
  Runner.reset_availability_cache!
  Runner.stub(:system_has_command?, available) do
    Runner.available? # Pre-populate cache
    yield
  end
end
```
