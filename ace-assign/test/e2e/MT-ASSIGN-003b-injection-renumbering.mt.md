...
Verify that ace-assign correctly handles child injection via `add --after X --child`, sibling injection with renumbering, and cascade renumbering of descendants when parent jobs are shifted.
...
# Create job.yaml and assignment with flat phases + initial children for injection testing
cat > "job.yaml" << 'EOF'
...
2. Verify file created with correct parent and provenance metadata
   ```bash
   ace-test-e2e-sh "$TEST_DIR" bash << 'SANDBOX'
   [ -f "$ASSIGNMENT_DIR/phases/010.03-setup-fixtures.ph.md" ] && echo "PASS: Job file 010.03-setup-fixtures.ph.md created" || echo "FAIL: Job file not created"
   grep -q 'parent:.*"010"' "$ASSIGNMENT_DIR/phases/010.03-setup-fixtures.ph.md" && echo "PASS: New job has parent: 010" || echo "FAIL: New job missing parent field"
...
1. Add a child under 010.03 (the shifted write-integration-tests)
   ```bash
   ace-test-e2e-sh "$TEST_DIR" bash << 'SANDBOX'
   ADD_OUTPUT=$($ACE_ASSIGN add integration-db-tests --after 010.03 --child -i "Database integration tests" 2>&1)
   ADD_EXIT=$?
   [ "$ADD_EXIT" -eq 0 ] && echo "PASS: Child created" || echo "FAIL: Child creation failed"
   [ -f "$ASSIGNMENT_DIR/phases/010.03.01-integration-db-tests.ph.md" ] && echo "PASS: 010.03.01 created" || echo "FAIL: 010.03.01 not created"
   SANDBOX
   ```
...
2. Inject sibling after 010.02 to trigger cascade renumbering
   ```bash
   ace-test-e2e-sh "$TEST_DIR" bash << 'SANDBOX'
   ADD_OUTPUT=$($ACE_ASSIGN add static-analysis --after 010.02 -i "Run static analysis" 2>&1)
   ADD_EXIT=$?
   echo "Exit code: $ADD_EXIT"
   echo "Output:"
   echo "$ADD_OUTPUT"
...
3. Verify files renamed correctly including cascaded child
   ```bash
   ace-test-e2e-sh "$TEST_DIR" bash << 'SANDBOX'
   [ -f "$ASSIGNMENT_DIR/phases/010.03-static-analysis.ph.md" ] && echo "PASS: New 010.03-static-analysis.ph.md exists" || echo "FAIL: New 010.03 missing"
   [ -f "$ASSIGNMENT_DIR/phases/010.04-write-integration-tests.ph.md" ] && echo "PASS: Old 010.03 is now 010.04" || echo "FAIL: Old 010.03 not at 010.04"
   ...
   [ ! -f "$ASSIGNMENT_DIR/phases/010.03.01-integration-db-tests.ph.md" ] && echo "PASS: Old 010.03.01 no longer exists" || echo "FAIL: Old child file still exists"
   grep -q 'renumbered_from:.*010.03.01' "$ASSIGNMENT_DIR/phases/010.04.01-integration-db-tests.ph.md" && echo "PASS: Child has renumbered_from: 010.03.01" || echo "FAIL: Child missing renumbered_from"
   SANDBOX
   ```
