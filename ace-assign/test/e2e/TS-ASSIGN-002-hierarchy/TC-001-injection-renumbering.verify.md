...
2. **Direct renumbering captured** — `first-sibling-inject.stdout` shows the inserted sibling under 010 and `step-listing-after-first-renumber.stdout` shows the shifted child at 010.03.
3. **Parent renumber metadata** — `renumbered-parent.stdout` contains `added_by: child_of:010`, `parent: "010"`, `renumbered_from: 010.02`, and `renumbered_at`.
