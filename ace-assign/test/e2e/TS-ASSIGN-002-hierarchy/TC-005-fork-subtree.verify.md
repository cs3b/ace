...
3. **Scoped subtree detected** — `status-scoped.stdout` mentions fork subtree root (020-subtree-a).
4. **Scoped view** — `status-scoped.stdout` shows only subtree phases: 020, 020.01 (onboard), 020.02 (plan-task), 020.03 (work-on-task). Scoped current phase is 020.01.
