...
5. **No state mutations** — `status-initial.json` and `status-after-scope.json` both preserve `current_step_line: "010 - precheck"`, and the raw status payload still shows subtree step statuses as pending after scoped inspection.
6. **Unscoped unchanged** — `status-after-scope.json` reports `current_step_line == "010 - precheck"`.

## Verdict

- **PASS**: Scoped status shows only subtree steps, resolves subtree current step, and causes no state mutations according to the before/after JSON oracles.
