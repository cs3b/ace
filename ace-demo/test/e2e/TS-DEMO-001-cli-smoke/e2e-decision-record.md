...
| TC-003 inline record dry-run preview | ADD | Confirms CLI-level dry-run output contract for inline mode and attach preview messaging in real command invocation context using stable public-surface assertions. | `test/fast/commands/cli_test.rb`, `test/fast/organisms/demo_recorder_test.rb`, `test/fast/molecules/inline_recorder_test.rb` |
| TC-004 attach missing --pr error | ADD | Verifies end-user exit code and required-argument guidance for missing `--pr` with stable CLI-contract assertions. | `test/fast/commands/cli_test.rb` |
| TC-005 record preset success artifact | ADD | Covers missing non-dry-run success path by validating preset recording produces a concrete output artifact. | `test/fast/commands/cli_test.rb`, `test/fast/organisms/demo_recorder_test.rb` |
