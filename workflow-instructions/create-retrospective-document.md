...
To create a comprehensive retrospective document for a completed release cycle (or other significant period).
This workflow focuses on synthesizing information from various sources, including completed tasks, ADRs,
individual reflection logs (outputs of former `self-reflect` actions), compact session logs, commit history,
and direct user/team input, to analyze what went well, what didn\'t, and identify actionable improvements.
...
3. **Gather & Synthesize Information:**
    ...
    Commit History: Use `git log --since=\"YYYY-MM-DD\" --until=\"YYYY-MM-DD\"` (or similar filters
    like `bin/gl`) for the period to understand the sequence of changes and effort distribution.
    Session Logs: Review `log-compact-session.md` outputs for daily progress, obstacles, and immediate context
    shifts.
    Individual reflection logs (previously associated with `self-reflect.md` outputs or other personal/team
    reflection practices) for deeper insights, learnings, and pain points.
    ADRs: Check any ADRs created or modified during the period for significant decisions and their
    rationale.
