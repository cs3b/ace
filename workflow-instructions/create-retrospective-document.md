...
1.  **Identify Target Release:** Confirm the path to the completed release directory in `docs-project/done/` (e.g., `docs-project/done/v1.2.0`).
...
3.  **Apply Template:** Copy the content from the standard retrospective template (`docs-dev/guides/prepare-release/v.x.x.x/reflections/_template.md`) into the new file.
4.  **Gather Context:** Review the release overview (`README.md`), completed tasks (`tasks/*.md`), reflection logs (`reflections/*.md`), and commit history for the release, all located within the `{release_dir}`.
...
## Input
- Path to the completed release directory (root-relative).
...
- [Self-Reflect Workflow Instruction](docs-dev/workflow-instructions/self-reflect.md) (Input for retrospectives)
