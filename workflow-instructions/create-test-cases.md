...
1.  **Analyze Requirements:** Review the feature/task description, implementation notes, and acceptance criteria (likely found in a task `.md` file within `docs-project`).
...
3.  **Categorize Tests:** Group the identified scenarios into test types (Unit, Integration, E2E, Performance, Security) based on `docs-dev/guides/testing.md`.
...
5.  **Structure Output:** Format the test cases clearly, potentially using a table or list structure. Use the template `docs-dev/guides/prepare-release/v.x.x.x/test-cases/_template.md` as a reference for structure.
