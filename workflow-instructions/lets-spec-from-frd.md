...
2.  **Determine Release Version & Path**:
    *   Check the project's current version (e.g., from `lib/aira/version.rb` or Git tags - check `docs-project/blueprint.md` for location).
    ...
    *   Create the release directory structure, typically starting in `docs-project/backlog/`:
        ...
        ```
    *   Copy standard templates (e.g., from `docs-dev/guides/prepare-release/v.x.x.x/`) into relevant subdirectories (e.g., copy `docs-dev/guides/prepare-release/v.x.x.x/decisions/_template.md` to `decisions/_template.md`).
...
3.  **Analyze FRD & Generate Tasks**:
    ...
    *   **Create Task Files**: Generate structured task files (`.md`) within the `{release_path}/tasks/` directory using the standard format (see `docs-dev/guides/project-management.md`).
...
4.  **Generate Release Overview (`README.md`)**:
    *   Create the main release overview file (`README.md` or `v.X.Y.Z-FeatureName.md`) within the `{release_path}`.
...
5.  **Communicate Results**:
    ...
    *   Show the generated directory structure using `tree {release_path}`.
    *   Indicate the location of the new release directory (usually in `docs-project/backlog/`).
    *   Suggest next steps (review tasks, refine plan, move to `docs-project/current/` when ready).

## Output / Success Criteria

**Output:**
- New release directory created in `docs-project/backlog/` (or specified location).
- Standard subdirectories (`docs`, `decisions`, `tasks`, `test-cases`, `user-experience`, `researches`) created within the release directory.
- Standard templates copied into subdirectories.
...
*   New release directory created in `docs-project/backlog/` (or `docs-project/current/` if specified).
...
- It integrates the principles regarding the artifacts needed for a Feature release outlined in `docs-dev/guides/ship-release.md`.
