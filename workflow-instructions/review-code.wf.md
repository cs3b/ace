...
Common patterns:
```bash
# Review entire source directory (auto-detects technology)
code-review code "src/**/*" --context auto --timeout 600

# Technology-specific patterns:
# Vue.js/JavaScript projects
code-review code "src/**/*" --context auto --timeout 600

# Ruby gem projects  
code-review code "lib/**/*.rb" --context auto --timeout 300

# Rust projects
code-review code "src/**/*.rs" --context auto --timeout 300

# Python projects
code-review code "**/*.py" --context auto --timeout 300

# Review recent changes (any technology)
code-review code HEAD~5..HEAD --context auto

# Review staged changes (any technology)
code-review code staged --context auto

# Combined code and test review
code-review "code tests" "src/**/*" --context auto --timeout 600

# Large codebase review with extended timeout
...
- **TARGET** (required): What to review
  ...
  - `"src/**/*"` - File patterns (use quotes for globs, common for JS/Vue projects)
  - `"lib/**/*.rb"` - Ruby gem source files
  - `"src/**/*.rs"` - Rust source files
  - `"**/*.py"` - Python files across all directories
  - `specific_file.ext` - Individual file
...
The `code-review` command automatically selects appropriate templates based on focus and technology context:

#### Technology Detection Priority
1. **Project Configuration**: Check `.claude/commands/review-code.md` for explicit technology context
2. **File Detection**: Auto-detect based on project files:
   - `package.json` + Vue.js dependencies → `vue.firebase` or `vue.general`
   - `Gemfile` + specific gems → `ruby.atom` or `ruby.rails`  
   - `Cargo.toml` → `rust.cli` or `rust.wasm`
   - `pyproject.toml`/`requirements.txt` → `python.django` or `python.general`
3. **Fallback**: Use generic templates if no technology detected

#### Template Selection by Focus and Technology
- **code**: Uses `dev-handbook/templates/review-code/system.{technology}.prompt.md`
  - Example: `system.vue.firebase.prompt.md` for Vue.js + Firebase projects
  - Example: `system.ruby.atom.prompt.md` for Ruby gem projects
- **tests**: Uses `dev-handbook/templates/review-test/system.{technology}.prompt.md`
- **docs**: Uses `dev-handbook/templates/review-docs/system.{technology}.prompt.md`
- **combined**: Uses multiple technology-specific templates and synthesizes results

#### Override Options
- Use `--system-prompt path/to/custom.prompt.md` to override automatic template selection
- Project-specific configurations in `.claude/commands/review-code.md` take precedence over auto-detection
