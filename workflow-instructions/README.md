...
```
draft-task → work-on-task → commit → fix-tests → review-code
```
...
```
review-code → synthesize-reviews → draft-task (for fixes)
```
...
1. **Create Urgent Task** (`draft-task`)
...
3. **Create Follow-up Tasks** (`draft-task`)
