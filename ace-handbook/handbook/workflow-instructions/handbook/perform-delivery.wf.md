...
* Access to required skills (`/ace_task_work`, `/ace_git_commit`, `/ace_git_create-pr`, `/ace_review_pr`)

## Project Context Loading

* Load task context via `ace-taskflow task <ref>` if task reference provided
...
* [ ] Execute implementation via `/ace_task_work` or inline instructions
* [ ] Commit all changes (`/ace_git_commit`)
* [ ] Release modified packages (`/ace_release` if applicable)
* [ ] Mark task done and push to remote

### Phase 2: PR & Initial Review

* [ ] Create or update PR (`/ace_git_create-pr`)
* [ ] Run initial review (`/ace_review_pr`)
* [ ] Implement HIGH/CRITICAL feedback immediately

### Phase 3: Deep Review Cycle

* [ ] Run deep code review (`/ace_review_pr preset: code-deep`)
* [ ] Implement MEDIUM+ severity feedback
* [ ] Run test suite (`ace-test-suite`)
* [ ] Commit fixes (`/ace_git_commit`)
...
2. **Create Session Todo List IMMEDIATELY:**
   ...
   **Standard Delivery Steps to Add:**
   ```
   1. Implement task (via /ace_task_work or inline)
   2. Commit all changes (/ace_git_commit)
   3. Release modified packages (/ace_release)
   4. Mark task done and push to remote
   5. Create/update PR (/ace_git_create-pr)
   6. Initial review (/ace_review_pr)
   7. Implement HIGH/CRITICAL feedback
   8. Deep review (/ace_review_pr preset: code-deep)
...
1. **Load Task Context (if task reference provided):**
   * Run `/ace_task_work <ref>` to load task context
...
4. **Commit Changes:**
   * Run `/ace_git_commit` to commit all changes
   * Use descriptive commit message

5. **Release Packages (if applicable):**
   * Run `/ace_release` to release modified packages
...
1. **Create or Update PR:**
   * Run `/ace_git_create-pr` to create pull request
...
1. **Run Deep Code Review:**
   * Run `/ace_review_pr preset: code-deep`
...
4. **Commit Fixes:**
   * Run `/ace_git_commit` to commit all fixes
...
```
/ace_handbook_perform-delivery 215.03
```
...
```
/ace_handbook_perform-delivery 215.03 --scope implementation
```
...
```
/ace_handbook_perform-delivery 215.03 --scope review
```
...
```
/ace_handbook_perform-delivery "Implement the feature described below, then create PR and review"
```
