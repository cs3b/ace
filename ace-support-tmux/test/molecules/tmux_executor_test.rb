...
class TmuxExecutorTest < Minitest::Test
  def setup
    @executor = Ace::Support::Tmux::Molecules::TmuxExecutor.new
  end

  def test_capture_returns_execution_result
    result = @executor.capture(["echo", "hello"])

    assert_instance_of Ace::Support::Tmux::Molecules::ExecutionResult, result
  ...
  def test_execution_result_attributes
    result = Ace::Support::Tmux::Molecules::ExecutionResult.new(
      stdout: "output",
      stderr: "error",
      success: false,
      exit_code: 2
