...
class WindowModelTest < Minitest::Test
  def test_default_initialization
    window = Ace::Support::Tmux::Models::Window.new
  ...
  def test_initialization_with_values
    pane = Ace::Support::Tmux::Models::Pane.new(commands: ["vim"])
    window = Ace::Support::Tmux::Models::Window.new(
      name: "editor",
  ...
  def test_focus_predicate
    assert Ace::Support::Tmux::Models::Window.new(focus: true).focus?
    refute Ace::Support::Tmux::Models::Window.new(focus: false).focus?
    refute Ace::Support::Tmux::Models::Window.new.focus?
  end
  ...
  def test_to_h_omits_nil_optional_fields
    window = Ace::Support::Tmux::Models::Window.new(name: "shell")
    hash = window.to_h
  ...
  def test_options_passthrough
    ...
    window = Ace::Support::Tmux::Models::Window.new(options: opts)
    assert_equal opts, window.options
  end

  def test_to_h_includes_options
    opts = { "main-pane-width" => "40%" }
    window = Ace::Support::Tmux::Models::Window.new(name: "main", options: opts)
  ...
  def test_nested_layout_with_tree
    pane = Ace::Support::Tmux::Models::Pane.new(commands: ["vim"])
    tree = Ace::Support::Tmux::Models::LayoutNode.new(
      direction: :horizontal,
      children: [Ace::Support::Tmux::Models::LayoutNode.new(pane: pane)]
    )
    window = Ace::Support::Tmux::Models::Window.new(layout_tree: tree, panes: [pane])
