...
### Fixed
- Added direct archive filesystem proof to `TS-IDEA-001` so the archive lifecycle E2E validates the moved `.idea.s.md` file instead of relying on CLI output alone.
...
### Fixed
- **ace-idea v0.18.1**: Bumped dependency constraints to currently available `~>` ranges on RubyGems and updated release metadata after dependency synchronization.
