...
2. **Saved report is usable** — `saved-report.json` includes token entries with `raw_value` fields required by remediation commands.
3. **Revoke step is explicit and safe** — revoke command executes with `--scan-file` and either:
...
4. **Rewrite dry-run is safe** — rewrite dry-run exits successfully and `before-head.txt` equals `after-head.txt`.
