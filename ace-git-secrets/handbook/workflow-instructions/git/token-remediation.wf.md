...
1. **Scan Git history for tokens:**
   ```bash
   # Full history scan (saves report to .cache/ace-git-secrets/)
   ...
   `.cache/ace-git-secrets/<timestamp>-report.json`. This report can be
...
11. **Revoke tokens immediately** (before they can be exploited):
   ...
   ace-git-secrets revoke --scan-file .cache/ace-git-secrets/<timestamp>-report.json
...
15. **Execute history rewrite:**
   ...
   ace-git-secrets rewrite-history --scan-file .cache/ace-git-secrets/<timestamp>-report.json
