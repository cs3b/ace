...
class WorktreeConfigTest < Minitest::Test
  ...
  def test_auto_setup_upstream_default_true
    config = Ace::Git::Worktree::Models::WorktreeConfig.new({}, @project_root)
    assert_equal true, config.auto_setup_upstream?
  end

  def test_auto_setup_upstream_can_be_disabled
    config_data = {
      "git" => {
        "worktree" => {
          "task" => {
  ...
  def test_auto_create_pr_default_true
    config = Ace::Git::Worktree::Models::WorktreeConfig.new({}, @project_root)
    assert_equal true, config.auto_create_pr?
  end

  def test_auto_create_pr_can_be_disabled
    config_data = {
      "git" => {
        "worktree" => {
          "task" => {
