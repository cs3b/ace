...
3. **Task entries have metadata** — Task-associated entries contain task_id fields with values `q7w` and `r8x` and branch fields.
4. **Non-task entries differ** — The main worktree entry has null, missing, or empty task_id field.

## Verdict

- **PASS**: JSON is valid, task worktrees have compact `task_id` and branch metadata, and the main worktree lacks task metadata.
