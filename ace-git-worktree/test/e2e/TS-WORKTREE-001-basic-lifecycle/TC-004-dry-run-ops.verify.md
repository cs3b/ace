...
2. **Create dry-run shows plan** — create-dry.exit is `0`, and `create-dry.stdout` indicates what would be created (path or branch name) rather than only showing command usage/help.
3. **Create dry-run is no-op** — create-dry-check.txt confirms the planned child worktree directory does NOT exist after dry-run (checking only the parent `.ace-wt` directory is insufficient). An explicit missing-path result such as `MISSING` is valid evidence.
