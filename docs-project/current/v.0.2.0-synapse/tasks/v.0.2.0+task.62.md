...
### Planning Steps
* [ ] Examine current Query command implementation to identify all exit calls
...
* [ ] Locate the proper CLI entry point where exit should be handled
...
* [ ] Review current Query spec structure to understand test patterns
...
### Execution Steps
- [ ] Modify Query#call method to return status codes instead of calling exit
...
- [ ] Update CLI entry point to handle Query command status and call exit appropriately
...
- [ ] Update Query specs to work with new return-based approach
...
- [ ] Run RSpec suite to verify no random failures occur
...
- [ ] Verify production behavior remains unchanged by testing CLI commands manually
...
- [ ] Query#call method returns status codes (0 for success, 1 for error) instead of calling exit
- [ ] CLI entry point properly handles Query command status and calls exit with appropriate code
- [ ] RSpec suite runs consistently regardless of test execution order
- [ ] RSpec suite completes with expected number of examples (1033)
- [ ] All Query specs pass without random failures
- [ ] Production CLI behavior remains unchanged from user perspective
- [ ] Test suite can be run multiple times with different seeds without early termination
