...
- [ ] Design fuzzy search filtering mechanism for model listing
- [ ] Plan model listing data structure and API integration approach
...
- [ ] Add --model flag to llm-lmstudio-query command parser
...
- [ ] Create llm-gemini-models command with fuzzy search filtering
...
- [ ] Create llm-lmstudio-models command with fuzzy search filtering
...
- [ ] Add model override tests to existing integration test files
  > TEST: Verify Model Override in Gemini Integration Tests
  > Type: Action Validation
  > Assert: Gemini integration tests include model override scenarios (test with gemini-2.5-flash)
  > Command: grep -n "model.*gemini-2.5-flash" spec/integration/llm_gemini_query_integration_spec.rb
  > TEST: Verify Model Override in LM Studio Integration Tests
  > Type: Action Validation
  > Assert: LM Studio integration tests include model override scenarios (test with deepseek/deepseek-r1-0528-qwen3-8b)
  > Command: grep -n "model.*deepseek" spec/integration/llm_lmstudio_query_integration_spec.rb
...
- [ ] AC 1b: LM Studio command accepts --model flag with proper argument parsing
- [x] AC 2: Model parameter is passed through to GeminiClient (✅ completed)
- [ ] AC 2b: Model parameter is passed through to LMStudioClient
...
- [ ] AC 4b: Default model works for LM Studio (mistralai/devstral-small-2505)
- [ ] AC 5: Help text documents available models and usage examples
- [ ] AC 6: Model listing commands work with fuzzy search filtering
- [ ] AC 7: Model override functionality is tested in existing integration tests (llm_gemini_query_integration_spec.rb and llm_lmstudio_query_integration_spec.rb)
...
- Test models: Use gemini-2.5-flash for Gemini tests and deepseek/deepseek-r1-0528-qwen3-8b for LM Studio tests in VCR cassettes
