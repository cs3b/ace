...
module Ace
  module Prompt
    module Atoms
      ...
      # Delegates to ace-taskflow for task resolution to avoid duplicating logic
      #
      # Pure functions only - git I/O is handled by Molecules::GitBranchReader
      class TaskPathResolver
        # Find task directory by ID using ace-taskflow API
        ...
        def self.resolve(task_id)
          # Use ace-taskflow's TaskManager to resolve task
        ...
        def self.extract_from_branch(branch_name, patterns: nil)
          ...
          patterns ||= Ace::Prompt.config.dig("task", "branch_patterns")
          patterns ||= ['^(\d+(?:\.\d+)?)-']
