...
### User Experience
- **Input**: Package `ace-retro` with CLI entrypoints but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-retro` to scaffold the initial scenario structure for the chosen area code and context.
...
4. Ensure proper sandbox setup/teardown for test isolation and record package-scoped verification with `ace-test ace-retro`.
...
- **Slice Outcome**: `ace-retro` has initial value-gated E2E smoke coverage with documented scenario boundaries
- **Advisory Size**: small
...
- [ ] Package-scoped verification passes: `ace-test ace-retro`
...
Create initial E2E smoke coverage for `ace-retro`, adding only scenarios whose behavior requires real CLI/filesystem/subprocess execution.
