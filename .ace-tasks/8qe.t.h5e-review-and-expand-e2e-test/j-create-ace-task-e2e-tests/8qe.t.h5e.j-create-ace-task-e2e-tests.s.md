...
### User Experience
- **Input**: Package `ace-task` with CLI command(s) (`ace-task` with 7 commands) but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-task` to scaffold the E2E test directory structure and base scenario file.
2. Create happy-path E2E test scenarios covering the primary use cases of each CLI command (`ace-task` with 7 commands).
3. Ensure proper sandbox setup/teardown for test isolation.
4. Verify all E2E tests pass with `ace-test ace-task`.

### Success Criteria

- [ ] E2E test directory structure created in `ace-task/test/e2e/`
- [ ] Happy-path scenarios cover all CLI commands (`ace-task` with 7 commands)
...
- **Slice Outcome**: `ace-task` has a new E2E test suite covering its CLI commands
- **Advisory Size**: small
...
- [ ] All E2E tests pass: `ace-test ace-task`
...
Create initial E2E test coverage for `ace-task`, which has CLI commands but currently lacks E2E tests.

## Scope of Work

- Scaffold E2E test structure via `/as-e2e-create`
- Create happy-path scenarios for `ace-task` with 7 commands
