...
### User Experience
- **Input**: Package `ace-support-models` with CLI command(s) (`ace-models`, `ace-llm-providers`) but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-support-models` to scaffold the E2E test directory structure and base scenario file.
2. Create happy-path E2E test scenarios covering the primary use cases of each CLI command (`ace-models`, `ace-llm-providers`).
3. Ensure proper sandbox setup/teardown for test isolation.
4. Verify all E2E tests pass with `ace-test ace-support-models`.

### Success Criteria

- [ ] E2E test directory structure created in `ace-support-models/test/e2e/`
- [ ] Happy-path scenarios cover all CLI commands (`ace-models`, `ace-llm-providers`)
...
- **Slice Outcome**: `ace-support-models` has a new E2E test suite covering its CLI commands
- **Advisory Size**: small
...
- [ ] All E2E tests pass: `ace-test ace-support-models`
...
Create initial E2E test coverage for `ace-support-models`, which has CLI commands but currently lacks E2E tests.
