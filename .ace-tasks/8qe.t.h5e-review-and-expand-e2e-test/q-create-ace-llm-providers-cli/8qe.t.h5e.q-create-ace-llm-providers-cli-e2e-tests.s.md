...
### User Experience
- **Input**: Package `ace-llm-providers-cli` with CLI entrypoints but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-llm-providers-cli` to scaffold the initial scenario structure for the chosen area code and context.
...
4. Ensure proper sandbox setup/teardown for test isolation and record package-scoped verification with `ace-test ace-llm-providers-cli`.
...
- **Slice Outcome**: `ace-llm-providers-cli` has initial value-gated E2E smoke coverage with documented scenario boundaries
- **Advisory Size**: small
...
Create initial E2E smoke coverage for `ace-llm-providers-cli`, adding only scenarios whose behavior requires real CLI/filesystem/subprocess execution.
