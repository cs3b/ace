...
### User Experience
- **Input**: Package `ace-llm-providers-cli` with CLI command(s) (`ace-llm-providers-cli-check`) but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-llm-providers-cli` to scaffold the E2E test directory structure and base scenario file.
2. Create happy-path E2E test scenarios covering the primary use cases of each CLI command (`ace-llm-providers-cli-check`).
3. Ensure proper sandbox setup/teardown for test isolation.
4. Verify all E2E tests pass with `ace-test ace-llm-providers-cli`.

### Success Criteria

- [ ] E2E test directory structure created in `ace-llm-providers-cli/test/e2e/`
- [ ] Happy-path scenarios cover all CLI commands (`ace-llm-providers-cli-check`)
...
- **Slice Outcome**: `ace-llm-providers-cli` has a new E2E test suite covering its CLI commands
- **Advisory Size**: small
- **Context Dependencies**: `ace-llm-providers-cli` CLI command implementations, existing E2E test patterns in other packages

### Verification Plan

#### Integration / E2E Validation
- [ ] All E2E tests pass: `ace-test ace-llm-providers-cli`
...
Create initial E2E test coverage for `ace-llm-providers-cli`, which has CLI commands but currently lacks E2E tests.

## Scope of Work

- Scaffold E2E test structure via `/as-e2e-create`
- Create happy-path scenarios for `ace-llm-providers-cli-check`
