...
### User Experience
- **Input**: Package `ace-handbook` with CLI command(s) (`ace-handbook`) but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-handbook` to scaffold the E2E test directory structure and base scenario file.
2. Create happy-path E2E test scenarios covering the primary use cases of each CLI command (`ace-handbook`).
3. Ensure proper sandbox setup/teardown for test isolation.
4. Verify all E2E tests pass with `ace-test ace-handbook`.

### Success Criteria

- [ ] E2E test directory structure created in `ace-handbook/test/e2e/`
- [ ] Happy-path scenarios cover all CLI commands (`ace-handbook`)
...
- **Slice Outcome**: `ace-handbook` has a new E2E test suite covering its CLI commands
- **Advisory Size**: small
...
- [ ] All E2E tests pass: `ace-test ace-handbook`
...
Create initial E2E test coverage for `ace-handbook`, which has CLI commands but currently lacks E2E tests.
