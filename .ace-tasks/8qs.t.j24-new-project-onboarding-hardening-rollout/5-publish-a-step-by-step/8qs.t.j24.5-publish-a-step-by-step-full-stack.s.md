...
Publish one blessed full-stack onboarding path across root docs. The docs must use real gem names, include config initialization with `ace-framework`, sync agent assets with `ace-handbook`, and reflect the verified behavior from the earlier children rather than inventing new setup guidance.
...
- **Process:** The docs provide a full-stack Gemfile or `bundle add` story for development/test, install the real ACE gems needed for the workflow, initialize config, sync agent assets, configure providers, and show first commands to prove the setup works.
...
2. The docs use real gem names and must not tell users to add `gem "ace-framework"`.
3. The docs include `bundle install`, `ace-framework init`, and `ace-handbook sync` in the correct order.
4. The docs include provider setup and first commands to verify the system is working.
5. If the RubyGems-lag proof child still requires mitigation, the docs include the validated mitigation and when to apply it.
...
```ruby
# Gemfile / bundle add story uses real gem names from the ACE stack.
...
2. The path uses real gem names and correct command ordering.
3. The path includes config init, handbook sync, provider setup, and first-use commands.
4. The docs are derived from children `8qs.t.j24.0` through `8qs.t.j24.4`, not independent guesses.
