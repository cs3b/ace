...
status: done
priority: medium
created_at: "2026-03-15 11:26:08"
estimate: TBD
dependencies: []
...
bundle:
  presets: [project]
  files: [ace-git-worktree/test-e2e/scenarios]
needs_review: false
---

# Review & improve ace-git-worktree E2E tests

## Behavioral Specification

### User Experience
- **Input**: Existing E2E test suite for `ace-git-worktree` (2 scenarios, 13 test cases).
...
1. Run `/as-e2e-manage` on `ace-git-worktree` to run `review -> plan-changes -> rewrite`, producing a coverage matrix plus classified KEEP/MODIFY/REMOVE/CONSOLIDATE/ADD decisions.
...
4. Record package-scoped verification for `ace-test ace-git-worktree`.

### Success Criteria

- [x] Coverage matrix produced for `ace-git-worktree`
...
- **Slice Outcome**: `ace-git-worktree` E2E tests managed through review, plan, rewrite, and package-scoped verification
- **Advisory Size**: small
- **Context Dependencies**: Existing E2E test files in `ace-git-worktree/test-e2e/scenarios/`

### Verification Plan

#### Integration / E2E Validation
- [x] Package-scoped verification passes after rewrite: `ace-test ace-git-worktree`
...
Run the canonical E2E lifecycle for `ace-git-worktree`, ensuring review findings, rewrite decisions, and package-scoped verification are all captured consistently.

## Scope of Work

- Run the canonical lifecycle against existing 2 scenarios and 13 test cases
