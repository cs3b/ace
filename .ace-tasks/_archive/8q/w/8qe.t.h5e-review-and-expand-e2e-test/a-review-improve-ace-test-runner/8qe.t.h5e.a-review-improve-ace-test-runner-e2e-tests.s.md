...
status: done
priority: medium
created_at: "2026-03-15 11:26:20"
estimate: TBD
dependencies: []
...
bundle:
  presets: [project]
  files: [ace-test-runner/test-e2e/scenarios]
needs_review: false
---

# Review & improve ace-test-runner E2E tests

## Behavioral Specification

### User Experience
- **Input**: Existing E2E test suite for `ace-test-runner` (2 scenarios, 5 test cases).
...
1. Run `/as-e2e-manage` on `ace-test-runner` to run `review -> plan-changes -> rewrite`, producing a coverage matrix plus classified KEEP/MODIFY/REMOVE/CONSOLIDATE/ADD decisions.
...
4. Record package-scoped verification for `ace-test ace-test-runner`.

### Success Criteria

- [ ] Coverage matrix produced for `ace-test-runner`
...
- **Slice Outcome**: `ace-test-runner` E2E tests managed through review, plan, rewrite, and package-scoped verification
- **Advisory Size**: small
- **Context Dependencies**: Existing E2E test files in `ace-test-runner/test-e2e/scenarios/`

### Verification Plan

#### Integration / E2E Validation
- [ ] Package-scoped verification passes after rewrite: `ace-test ace-test-runner`
...
Run the canonical E2E lifecycle for `ace-test-runner`, ensuring review findings, rewrite decisions, and package-scoped verification are all captured consistently.

## Scope of Work

- Run the canonical lifecycle against existing 2 scenarios and 5 test cases
