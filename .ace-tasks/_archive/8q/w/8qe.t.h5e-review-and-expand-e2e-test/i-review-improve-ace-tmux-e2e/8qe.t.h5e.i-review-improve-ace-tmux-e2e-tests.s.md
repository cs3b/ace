...
status: done
priority: medium
created_at: "2026-03-15 11:26:26"
estimate: TBD
dependencies: []
...
bundle:
  presets: [project]
  files: [ace-tmux/test-e2e/scenarios]
needs_review: false
---

# Review & improve ace-tmux E2E tests

## Behavioral Specification

### User Experience
- **Input**: Existing E2E test suite for `ace-tmux` (1 scenario, 2 test cases).
...
1. Run `/as-e2e-manage` on `ace-tmux` to run `review -> plan-changes -> rewrite`, producing a coverage matrix plus classified KEEP/MODIFY/REMOVE/CONSOLIDATE/ADD decisions.
...
4. Record package-scoped verification for `ace-test ace-tmux`.

### Success Criteria

- [x] Coverage matrix produced for `ace-tmux`
...
- **Slice Outcome**: `ace-tmux` E2E tests managed through review, plan, rewrite, and package-scoped verification
- **Advisory Size**: small
- **Context Dependencies**: Existing E2E test files in `ace-tmux/test-e2e/scenarios/`

### Verification Plan

#### Integration / E2E Validation
- [x] Package-scoped verification passes after rewrite: `ace-test ace-tmux`
...
Run the canonical E2E lifecycle for `ace-tmux`, ensuring review findings, rewrite decisions, and package-scoped verification are all captured consistently.
