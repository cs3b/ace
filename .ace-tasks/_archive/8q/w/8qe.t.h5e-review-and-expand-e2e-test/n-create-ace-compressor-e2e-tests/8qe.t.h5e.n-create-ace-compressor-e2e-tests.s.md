...
### User Experience
- **Input**: Package `ace-compressor` with CLI entrypoints but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-compressor` to scaffold the initial scenario structure for the chosen area code and context.
...
4. Ensure proper sandbox setup/teardown for test isolation and record package-scoped verification with `ace-test ace-compressor`.

### Success Criteria

- [ ] E2E test directory structure created in `ace-compressor/test-e2e/scenarios/`
...
- **Slice Outcome**: `ace-compressor` has initial value-gated E2E smoke coverage with documented scenario boundaries
- **Advisory Size**: small
...
- [ ] Package-scoped verification passes: `ace-test ace-compressor`
...
Create initial E2E smoke coverage for `ace-compressor`, adding only scenarios whose behavior requires real CLI/filesystem/subprocess execution.
