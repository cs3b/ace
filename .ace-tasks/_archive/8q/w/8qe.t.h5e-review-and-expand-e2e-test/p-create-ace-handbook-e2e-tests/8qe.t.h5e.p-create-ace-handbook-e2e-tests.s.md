...
status: done
priority: medium
created_at: "2026-03-15 11:26:36"
estimate: TBD
dependencies: []
...
# Create ace-handbook E2E tests

## Behavioral Specification

### User Experience
- **Input**: Package `ace-handbook` with CLI entrypoints but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-handbook` to scaffold the initial scenario structure for the chosen area code and context.
...
4. Ensure proper sandbox setup/teardown for test isolation and record package-scoped verification with `ace-test ace-handbook`.

### Success Criteria

- [x] E2E test directory structure created in `ace-handbook/test-e2e/scenarios/`
...
- **Slice Outcome**: `ace-handbook` has initial value-gated E2E smoke coverage with documented scenario boundaries
- **Advisory Size**: small
- **Context Dependencies**: `ace-handbook` CLI command implementations, existing E2E test patterns in other packages

### Verification Plan

#### Integration / E2E Validation
- [x] Package-scoped verification passes: `ace-test ace-handbook`
- [x] Scenario objectives and unit-coverage-reviewed fields reflect genuine E2E-only behavior

#### Verification Commands
- [x] `ace-test ace-handbook`

## Objective

Create initial E2E smoke coverage for `ace-handbook`, adding only scenarios whose behavior requires real CLI/filesystem/subprocess execution.

## Scope of Work

- Scaffold E2E test structure via `/as-e2e-create`
- Create happy-path scenarios for `ace-handbook`
