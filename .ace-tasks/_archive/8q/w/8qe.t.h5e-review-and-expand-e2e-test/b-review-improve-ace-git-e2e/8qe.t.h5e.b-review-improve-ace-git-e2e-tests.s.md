...
status: done
priority: medium
created_at: "2026-03-15 11:26:21"
estimate: TBD
dependencies: []
...
bundle:
  presets: [project]
  files: [ace-git/test-e2e/scenarios]
needs_review: false
---

# Review & improve ace-git E2E tests

## Behavioral Specification

### User Experience
- **Input**: Existing E2E test suite for `ace-git` (1 scenario, 4 test cases).
...
1. Run `/as-e2e-manage` on `ace-git` to run `review -> plan-changes -> rewrite`, producing a coverage matrix plus classified KEEP/MODIFY/REMOVE/CONSOLIDATE/ADD decisions.
...
4. Record package-scoped verification for `ace-test ace-git`.

### Success Criteria

- [ ] Coverage matrix produced for `ace-git`
...
- **Slice Outcome**: `ace-git` E2E tests managed through review, plan, rewrite, and package-scoped verification
- **Advisory Size**: small
- **Context Dependencies**: Existing E2E test files in `ace-git/test-e2e/scenarios/`

### Verification Plan

#### Integration / E2E Validation
- [ ] Package-scoped verification passes after rewrite: `ace-test ace-git`
...
Run the canonical E2E lifecycle for `ace-git`, ensuring review findings, rewrite decisions, and package-scoped verification are all captured consistently.
