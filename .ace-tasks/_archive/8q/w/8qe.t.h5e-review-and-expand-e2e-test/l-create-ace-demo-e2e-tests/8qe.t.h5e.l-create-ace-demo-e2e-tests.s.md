...
### User Experience
- **Input**: Package `ace-demo` with CLI entrypoints but no existing E2E test coverage.
...
1. Run `/as-e2e-create` on `ace-demo` to scaffold the initial scenario structure for the chosen area code and context.
...
4. Ensure proper sandbox setup/teardown for test isolation and record package-scoped verification with `ace-test ace-demo`.

### Success Criteria

- [x] E2E test directory structure created in `ace-demo/test-e2e/scenarios/`
...
- **Slice Outcome**: `ace-demo` has initial value-gated E2E smoke coverage with documented scenario boundaries
- **Advisory Size**: small
...
- [x] Package-scoped verification passes: `ace-test ace-demo`
...
Create initial E2E smoke coverage for `ace-demo`, adding only scenarios whose behavior requires real CLI/filesystem/subprocess execution.
