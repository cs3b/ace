...
### Fixed
- Hardened the seeded-cache providers E2E to capture the exact cache file shape and `XDG_CACHE_HOME` binding used by `ace-llm-providers list/show`.
...
### Fixed
- **ace-support-models v0.9.1**: Bumped dependency constraints to currently available `~>` ranges on RubyGems and updated release metadata after dependency synchronization.
