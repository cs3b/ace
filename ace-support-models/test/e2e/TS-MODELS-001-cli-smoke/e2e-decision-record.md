...
| TC-003 providers list/show after public sync | KEEP | Confirms user-visible output and exit semantics from real `ace-llm-providers` invocations after public `ace-models sync` setup. | `test/fast/commands/providers_commands_test.rb`, `test/feat/cli_integration_test.rb` |
...
| TC-005 cache status after sync | KEEP | Verifies public cache status reporting contract from real `ace-models` invocation after sync. | `test/fast/commands/cache_commands_test.rb`, `test/feat/cli_integration_test.rb` |
| TC-006 cache diff after refresh | KEEP | Verifies real diff output contract across sequential sync runs with changed model metadata. | `test/fast/commands/cache_commands_test.rb`, `test/feat/cli_integration_test.rb` |
