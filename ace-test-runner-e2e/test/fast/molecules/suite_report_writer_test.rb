...
class SuiteReportWriterTest < Minitest::Test
  ...
  def test_llm_response_is_written_to_file
    Dir.mktmpdir do |tmpdir|
      ...
      assert_match(/Tighten the verifier contract/, content)
  ...
  def test_llm_failure_falls_back_to_static_template
    Dir.mktmpdir do |tmpdir|
      ...
      assert_match(/## Summary Table/, content)
      assert_match(/TS-TEST-001/, content)
      assert_match(/TS-TEST-002/, content)
      assert_match(/## Reports Table/, content)
  ...
  def test_static_fallback_overall_line_shows_tc_stats
    Dir.mktmpdir do |tmpdir|
      ...
      content = File.read(path)
      assert_match(/## Overall Line/, content)
      assert_match(/\*\*Overall:\*\* 3\/4 test cases passed \(75%\)/, content)
    end
  end
  ...
  def test_deterministic_sections_ignore_hallucinated_titles_and_failed_tcs
    Dir.mktmpdir do |tmpdir|
      results, scenarios = build_results_and_scenarios(tmpdir)
      stub_llm_query(<<~MD)
        ## Summary Table
        ...
        |---|---|---:|---:|---:|---:|
        | TS-TEST-001 | Wrong Title | pass | 2 | 0 | 2 |
        | TS-TEST-002 | Wrong Other Title | partial | 1 | 1 | 2 |

        ## Failed Tests

        ### TS-TEST-002

        - `TC-999` — hallucinated failure
      MD
      ...
      content = File.read(path)
      assert_match(/\| TS-TEST-001 \| Test One \| Pass \| 2 \| 0 \| 2 \|/, content)
      assert_match(/\| TS-TEST-002 \| Test Two \| Fail \| 1 \| 1 \| 2 \|/, content)
      assert_match(/`TC-002`/, content)
      refute_match(/Wrong Title/, content)
      refute_match(/TC-999/, content)
    end
  end

  def test_llm_missing_narrative_sections_still_writes_single_deterministic_overall_line
    Dir.mktmpdir do |tmpdir|
      results, scenarios = build_results_and_scenarios(tmpdir)
      ...
      content = File.read(path)
      assert_equal 1, content.scan(/\*\*Overall:\*\*/).size
      assert_match(/\*\*Overall:\*\* 3\/4 test cases passed \(75%\)/, content)
    end
  end

  def test_llm_fallback_text_is_placed_in_narrative_section_only
    Dir.mktmpdir do |tmpdir|
      results, scenarios = build_results_and_scenarios(tmpdir)
      stub_llm_query("# Report\n\nCorrect analysis.")
      ...
      assert_match(/## Positive Observations/, content)
    end
  end
