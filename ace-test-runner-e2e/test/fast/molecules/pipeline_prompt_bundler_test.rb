...
class PipelinePromptBundlerTest < Minitest::Test
  ...
  def test_prepare_runner_bundles_only_selected_test_cases
    Dir.mktmpdir do |tmpdir|
      ...
      assert_includes system_prompt, "Never background commands or start dependent verification captures"
      assert_includes system_prompt, "write that command's `.exit` file before running any list/status/fs-check/tmux verification"
  ...
  def test_prepare_verifier_embeds_artifacts_and_verify_criteria
    Dir.mktmpdir do |tmpdir|
      ...
      assert_includes content, "File metadata"
      assert_includes content, "mtime="
