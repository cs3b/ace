...
2.  **Context is Key**: Assume the AI might not have the full immediate context. Reference relevant guides, project documents (`blueprint.md`, `architecture.md`), or existing code patterns.
...
*   Reference other guides or commands where necessary (e.g., "Refer to `guides/version-control.md` for commit message format").
...
*   List relevant guides or project documents that provide additional context.
...
Review existing workflow instructions in `coding-agent-workflow-toolkit/workflow-instructions/` like:
*   `load-env.md`: Focuses on context gathering.
*   `lets-spec-from-pr-comments.md`: Complex process with specific inputs (fetched comments) and outputs (structured tasks).
*   `lets-commit.md`: Guides a specific, common developer action referencing external standards.
*   `self-reflect.md`: Defines a meta-process for analysis and improvement.
...
*   **Reference Explicitly:** Use full paths to files or other instructions where possible (`guides/coding-standards.md`, `workflow-instructions/load-env.md`).
