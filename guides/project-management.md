...
- **`bin/rc` (`get-current-release-path`)**: Determines the appropriate directory for storing newly created tasks and returns version information. Returns the path to the current release directory (e.g., `docs-project/current/v.X.Y.Z-codename`) and version string (e.g., `v.X.Y.Z`) when a current release exists, or the backlog tasks path (`docs-dev/backlog/tasks`) and empty version when no current release is detected. This tool is essential for automated task creation workflows.
