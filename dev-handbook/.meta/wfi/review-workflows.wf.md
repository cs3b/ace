...
Systematically review multiple workflow instruction files for compliance with standards, consistency, and quality. This meta-workflow provides batch review capabilities for handbook maintainers to ensure workflow instructions follow established patterns and maintain high quality across the entire collection.
...
* Load example workflow: `wfi://plan-task`
...
1. **Define Review Scope:**
   * Determine which workflows to review:
     - All workflows in `dev-handbook/workflow-instructions/`
...
2. **Gather Workflow Inventory:**
   * List all workflow files to review:
     ...
     ```bash
     find dev-handbook/workflow-instructions -name "*.wf.md" -type f
     ```
...
for file in dev-handbook/workflow-instructions/*.wf.md; do
...
grep -r "workflow-instructions/" dev-handbook/workflow-instructions/ --include="*.wf.md"

# Check for broken template embedding
grep -r "<template" dev-handbook/workflow-instructions/ --include="*.wf.md" | grep -v "path="
```
...
> "Review all workflow instructions in dev-handbook/workflow-instructions/ for compliance with current standards and identify any inconsistencies or improvement opportunities"