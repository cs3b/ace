    ...
    module Atoms
      ...
      class PathExpander
        ...
        def self.for_file(source_file, project_root: nil)
          ...
          # Ace::Config::Molecules::ProjectRootFinder and pass the result
        ...
        # @note Supports Windows paths via File::ALT_SEPARATOR
        def self.absolute?(path)
          return false if path.nil?

          path_str = path.to_s
          return true if Pathname.new(path_str).absolute?

          # Windows support: check for backslash-prefixed paths
          !!(File::ALT_SEPARATOR && path_str.start_with?(File::ALT_SEPARATOR))
        end
