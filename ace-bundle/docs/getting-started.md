...
You can also use `bundle.sources` when a preset is generated by another tool or needs one mixed, ordered list:

```markdown
---
description: My generated context
bundle:
  sources:
    - file: README.md
...
Each source entry is normalized to the same file, command, or preset behavior used by the regular preset keys.
