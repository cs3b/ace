...
env -i \
  ...
  BUNDLE_WITHOUT="" \
  bundle env > results/tc/03/bundle-env.stdout 2> results/tc/03/bundle-env.stderr
```
3. Run `bundle install --full-index` with all Bundler-preserved variables set to the local surface:
```bash
env -i HOME="$HOME" PATH="$PATH" \
  ...
  PROJECT_ROOT_PATH="$PWD" \
  BUNDLE_WITHOUT="" \
  bundle install --full-index > results/tc/03/fullindex.stdout 2>&1
echo $? > results/tc/03/fullindex.exit
```
4. If install succeeds:
   - Capture local install state and env:
   ```bash
   env -i HOME="$HOME" PATH="$PATH" \
     BUNDLE_GEMFILE="$PWD/Gemfile" \
     BUNDLE_APP_CONFIG="$PWD/results/tc/03/.bundle/config" \
   ...
   env -i HOME="$HOME" PATH="$PATH" \
     ...
     GEM_HOME="$PWD/results/tc/03/.gem" \
     PROJECT_ROOT_PATH="$PWD" \
     bundle exec ruby - <<'RUBY' > results/tc/03/version-check.stdout 2> results/tc/03/version-check.stderr
