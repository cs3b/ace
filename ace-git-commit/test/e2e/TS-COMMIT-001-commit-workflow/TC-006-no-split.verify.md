...
- **Artifacts exist** — `results/tc/06/` contains setup evidence, command
  captures, and git log/show captures.
- **Zero exit code** — command exit capture is `0`.
- **Single commit** — Git log shows exactly one new commit for this goal.
- **Both packages** — `git show --stat` shows files from both `pkg-a` and
  `pkg-b` in the same commit.
