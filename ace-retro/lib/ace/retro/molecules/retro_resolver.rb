...
require_relative "retro_scanner"

module Ace
  module Retro
    module Molecules
      # Wraps ShortcutResolver for raw retro IDs (no type marker).
      # Resolves 3-char suffix shortcuts, full 6-char IDs.
      # Explicitly detects and warns on ambiguity collisions.
      class RetroResolver
        # @param root_dir [String] Root directory containing retros
        def initialize(root_dir)
          @root_dir = root_dir
          @scanner = RetroScanner.new(root_dir)
        end
        ...
        def resolve(ref, warn_on_ambiguity: true)
          ...
          on_ambiguity = if warn_on_ambiguity
            ->(matches) {
              ...
              warn "Warning: Ambiguous shortcut '#{ref}' matches #{matches.size} retros: #{ids}. Using most recent."
            }
          end
