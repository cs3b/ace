...
3. **Correct extension resolved** — The stdout contains a resolved workflow path including `.wf.md`, proving the wfi:// protocol uses its own shorthand extension for inference.
4. **Protocol-specific shorthand** — The resolved path must be workflow-specific (`.wf.md`) and must not resolve to guide shorthand output (`.g.md`).
...
- **PASS**: wfi:// resolution succeeded with a resolved `.wf.md` workflow path, protocol-specific shorthand behavior is clear, and captures are complete.
