...
class NavigationIntegrationTest < Minitest::Test
  ...
  def test_full_navigation_flow_from_uri_to_resource
    Dir.chdir(@test_dir) do
      ...
      uri_parser = Ace::Support::Nav::Atoms::UriParser.new(config_loader: config_loader)
      scanner = Ace::Support::Nav::Molecules::ProtocolScanner.new(config_loader: config_loader)
      resolver = Ace::Support::Nav::Organisms::ResourceResolver.new(
        uri_parser: uri_parser,
        protocol_scanner: scanner
      )

      # Parse and resolve
      uri = Ace::Support::Nav::Models::ResourceUri.new("wfi://setup", config_loader: config_loader)
  ...
  def test_cascade_search_finds_resources_across_sources
    Dir.chdir(@test_dir) do
      navigator = Ace::Support::Nav::Navigator.new
  ...
  def test_legacy_compatibility_handbook_source
    Dir.chdir(@test_dir) do
      ...
      scanner = Ace::Support::Nav::Molecules::HandbookScanner.new

      sources = scanner.scan_all_sources
      assert sources.all? { |s| s.is_a?(Ace::Support::Nav::Models::HandbookSource) }
