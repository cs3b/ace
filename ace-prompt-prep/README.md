...
```bash
# Create a prompt
mkdir -p .ace-local/prompt-prep/prompts
echo "Review this code for security issues" > .ace-local/prompt-prep/prompts/the-prompt.md
...
cat > .ace-local/prompt-prep/prompts/feature-review.md << 'EOF'
---
bundle:
  enabled: true
  sources:
...
1. Reads `.ace-local/prompt-prep/prompts/the-prompt.md` (or task-specific directory)
