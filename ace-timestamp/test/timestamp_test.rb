...
module Ace
  class TimestampTest < Minitest::Test
    ...
    def test_encode_convenience_method
      ...
      result = Ace::Timestamp.encode(time)
    ...
    def test_encode_with_year_zero_override
      ...
      result = Ace::Timestamp.encode(time, year_zero: 2025)

      assert_equal "000000", result
    end

    def test_decode_convenience_method
      result = Ace::Timestamp.decode("000000")
    ...
    def test_decode_with_year_zero_override
      result = Ace::Timestamp.decode("000000", year_zero: 2025)

      assert_equal 2025, result.year
    end

    def test_valid_convenience_method
      assert Ace::Timestamp.valid?("abc123")
      refute Ace::Timestamp.valid?("invalid!")
    end

    def test_detect_format_convenience_method
      assert_equal :compact, Ace::Timestamp.detect_format("abc123")
      assert_equal :timestamp, Ace::Timestamp.detect_format("20250101-120000")
      assert_nil Ace::Timestamp.detect_format("invalid")
    end

    def test_now_generates_current_time_id
      result = Ace::Timestamp.now

      assert_equal 6, result.length
      assert_match(/\A[0-9a-z]{6}\z/, result)

      # Should decode to approximately current time
      decoded = Ace::Timestamp.decode(result)
      assert_in_delta Time.now.utc.to_i, decoded.to_i, 10
    end

    def test_config_returns_hash
      config = Ace::Timestamp.config
    ...
    def test_encode_decode_roundtrip
      ...
      encoded = Ace::Timestamp.encode(original)
      decoded = Ace::Timestamp.decode(encoded)
    ...
    def test_module_error_class_exists
      assert_kind_of Class, Ace::Timestamp::Error
      assert Ace::Timestamp::Error < StandardError
    end
  end
end
