      ...
      class CompactIdEncoder
        ...
        class << self
          ...
          def encode(time, year_zero: DEFAULT_YEAR_ZERO, alphabet: DEFAULT_ALPHABET)
            ...
            # - Leap seconds (second = 60) could occur in UTC
            ...
            (encode_value(months_offset, 2, alphabet) +
              encode_value(day, 1, alphabet) +
              encode_value(block, 1, alphabet) +
              encode_value(precision, 2, alphabet)).freeze
          end
          ...
          def validate_component_ranges!(months_offset, day, block, precision)
            if months_offset > 1295
              raise ArgumentError, "Month offset #{months_offset} exceeds maximum (1295 = 108 years)"
            end

            if day > 30
              raise ArgumentError, "Day value #{day} exceeds maximum (30 for calendar days 1-31)"
            end

            if block > 35
              raise ArgumentError, "Block value #{block} exceeds maximum (35 for 36 blocks/day)"
            end

            if precision > 1295
              raise ArgumentError, "Precision value #{precision} exceeds maximum (1295 = 36^2 - 1)"
            end
          end
        end
      end
