...
> [Strategic Planning Guide](ace-handbook/handbook/guides/strategic-planning.g.md).
...
`/ace-task-drafts` workflow to process items from this internal backlog and integrate them as structured
...
- Links to relevant documentation (e.g., `ace-handbook/handbook/guides/coding-standards.g.md`), ADRs (`docs/decisions/...`),
...
1. **Archiving**: After a release is successfully published, its directory (e.g., `v.0.2.0-active-sprint`) is moved
   ...
   executed through the `/ace-release-publish` workflow.
...
- **`release-manager current`**: Determines the appropriate directory for storing newly created
  ...
  or the backlog tasks path (`.ace-taskflow/backlog/tasks`) and empty version when no current release is detected.
  This tool is essential for automated task creation workflows.
- **`ace-taskflow lint`**: Validates task files for correct ID format, required fields (status, priority),
  valid values, and other conventions.
...
- Any decisions made or significant learnings are documented (e.g., in the task file, an ADR in
  `docs/decisions/`, or a `self-reflect` log).
...
Tasks and release structures are typically generated through one of three primary workflows:
...
2. **Test-Driven Development (AI-Assisted)**:
    - **Write Tests (Test Phase in `work-on-task`)**: Guide the AI to generate failing tests based on the planned
      structure and acceptance criteria using the testing phase described in the `/ace-task-work` workflow.
...
3. **Documentation & Committing**: Document decisions within the code or task file. Guide the AI to generate
   or help format commit messages according to conventions using the `/ace-git-commit` workflow. *Review commit
...
4. **Review & Reflection**: After completing a task or a significant work segment, engage in self-reflection.
   ...
   and capture learnings. Use the `/ace-retro-create` workflow to capture these insights. These individual
...
critically. Refer to `ace-handbook/handbook/guides/coding-standards.g.md` for more on AI collaboration best practices.
...
- [Coding Standards](ace-handbook/handbook/guides/coding-standards.g.md)
- [Testing Guidelines](ace-test-runner/handbook/guides/testing.g.md)
- [Documentation Standards](ace-docs/handbook/guides/documentation.g.md)
- [Publish Release Process](ace-taskflow/handbook/guides/release-publish.g.md) (Final deployment and archival)
- [Release Codenames Guide](ace-taskflow/handbook/guides/project-management/release-codenames.g.md) (Philosophy and approach for release naming)
- [Error Handling](ace-handbook/handbook/guides/error-handling.g.md)
- [Performance](ace-handbook/handbook/guides/performance.g.md)
- [Security](ace-git-secrets/handbook/guides/security.g.md)
- [Troubleshooting Workflow](ace-handbook/handbook/guides/debug-troubleshooting.g.md)
- [Strategic Planning Guide](ace-handbook/handbook/guides/strategic-planning.g.md)
