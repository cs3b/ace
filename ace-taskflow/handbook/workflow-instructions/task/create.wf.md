...
**Note**: Implementation planning (plan-task) is no longer part of task creation. Planning happens JIT via ace-assign phases just before implementation begins. Use `/ace:review-task` to validate and promote drafts to pending.
...
- [ ] Task ready for review via `/ace:review-task`

## Next Steps After Task Creation

- Run `/ace:review-task` to validate the draft and promote to pending
