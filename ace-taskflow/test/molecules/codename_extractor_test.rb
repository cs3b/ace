...
class CodenameExtractorTest < AceTaskflowTestCase
  ...
  def test_extract_falls_back_to_first_md_when_no_readme
    Dir.mktmpdir do |tmpdir|
      # When no README.md exists, falls back to first .md file
      # (matches StatsFormatter behavior for consistency)
      File.write(File.join(tmpdir, "zzz.md"), "# v.0.9.0 Wrong File\n")
      File.write(File.join(tmpdir, "aaa.md"), "# v.0.9.0 Another File\n")

      result = @extractor.extract(tmpdir)
      # Dir.glob order is filesystem-dependent, so just verify we get one of them
      assert_includes ["Wrong File", "Another File"], result
    end
  end
