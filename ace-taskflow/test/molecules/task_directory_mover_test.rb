...
class TaskDirectoryMoverTest < AceTaskflowTestCase
  ...
  def test_move_to_deferred
    with_test_project do |dir|
      Dir.chdir(dir) do
        ...
        # Move to deferred
        result = @mover.move_to_deferred(task_file)

        assert result[:success], "Should succeed: #{result[:message]}"
        expected_path = File.join(tasks_dir, "_deferred", "001-test-task", "001-test-task.s.md")
  ...
  def test_move_to_deferred_idempotent
    with_test_project do |dir|
      Dir.chdir(dir) do
        # Create test task directory structure already in deferred
        tasks_dir = File.join(dir, ".ace-taskflow", "v.0.9.0", "t")
        deferred_dir = File.join(tasks_dir, "_deferred", "001-test-task")
        ...
        assert result[:success], "Should succeed: #{result[:message]}"
        assert_match(/already in _deferred/, result[:message])
  ...
  def test_move_to_deferred_with_nil_path
    result = @mover.move_to_deferred(nil)
    refute result[:success]
    assert_match(/not provided/i, result[:message])
  end

  def test_move_to_deferred_with_nonexistent_path
    result = @mover.move_to_deferred("/nonexistent/path")
    refute result[:success]
    assert_match(/not found/i, result[:message])
  end

  def test_restore_from_deferred
    with_test_project do |dir|
      Dir.chdir(dir) do
        ...
        result = @mover.restore_from_deferred(task_file)
  ...
  def test_restore_from_deferred_with_nil_path
    result = @mover.restore_from_deferred(nil)
    refute result[:success]
    assert_match(/not provided/i, result[:message])
  end

  def test_restore_from_deferred_with_nonexistent_path
    result = @mover.restore_from_deferred("/nonexistent/path")
    refute result[:success]
    assert_match(/not found/i, result[:message])
  end

  def test_restore_from_deferred_with_non_deferred_path
    with_test_project do |dir|
      Dir.chdir(dir) do
        ...
        refute result[:success]
        assert_match(/not in _deferred/i, result[:message])
      end
    end
  end

  def test_restore_from_deferred_with_target_already_exists
    with_test_project do |dir|
      Dir.chdir(dir) do
        ...
        result = @mover.restore_from_deferred(deferred_file)
