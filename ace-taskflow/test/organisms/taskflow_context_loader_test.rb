...
class TaskflowContextLoaderTest < AceTaskflowTestCase
  ...
  def test_load_includes_release_info
    # Create tasks matching expected statistics: 10 total, 7 done (5 done + 2 completed)
    mock_tasks_for_stats = [
      { id: "v.0.9.0+task.001", status: "done" },
      { id: "v.0.9.0+task.002", status: "done" },
      { id: "v.0.9.0+task.003", status: "done" },
      { id: "v.0.9.0+task.004", status: "done" },
      { id: "v.0.9.0+task.005", status: "done" },
      { id: "v.0.9.0+task.006", status: "completed" },
      { id: "v.0.9.0+task.007", status: "completed" },
      { id: "v.0.9.0+task.008", status: "in-progress" },
      { id: "v.0.9.0+task.009", status: "in-progress" },
      { id: "v.0.9.0+task.010", status: "in-progress" }
    ]
    Ace::Taskflow::Molecules::ReleaseResolver.stub :new, mock_release_resolver do
      Ace::Taskflow::Molecules::TaskLoader.stub :new, mock_task_loader_with_tasks(mock_tasks_for_stats) do
        @loader.stub :detect_task_pattern_from_branch, nil do
          context = @loader.load
  ...
  def test_detect_task_pattern_from_branch
    ...
    Ace::Git::Molecules::BranchReader.stub :current_branch, mock_branch do
  ...
  def mock_release_resolver
    ...
    # Required by TaskManager.resolve_release_path
    def resolver.find_release(identifier)
  ...
  def mock_task_loader_no_task
    ...
    # Required by TaskManager.list_tasks when glob patterns are used
    def loader.load_tasks_with_glob(release_path, glob_patterns)
      []
    end
    loader
  end
  ...
  def mock_task_loader_with_task(task)
    ...
    # Required by TaskManager.list_tasks when glob patterns are used
    loader.define_singleton_method(:load_tasks_with_glob) do |release_path, glob_patterns|
      [task]
    end
    loader
  end
  ...
  def mock_task_loader_with_tasks(tasks = nil)
    ...
    # Required by TaskManager.list_tasks when glob patterns are used
    loader.define_singleton_method(:load_tasks_with_glob) { |_, _| tasks }
    loader
  end
end
