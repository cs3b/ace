  ...
  module Taskflow
    module Molecules
      ...
      class CodenameExtractor
        ...
        # Only returns README.md - we don't fall back to arbitrary files to avoid
        # extracting codenames from unintended sources
        # @param release_path [String] Path to release directory
        # @return [String, nil] Path to README.md or nil if not found
        def find_main_file(release_path)
          md_files = Dir.glob(File.join(release_path, "*.md"))
          return nil if md_files.empty?

          # Only use README.md as the canonical source for codenames
          md_files.find { |f| File.basename(f).downcase == "readme.md" }
        end
