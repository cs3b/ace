    ...
    module CLI
      module Commands
        module IdeaSubcommands
          # dry-cli Command class for idea park nested subcommand
          #
          # Moves an idea to the maybe/ subdirectory for ideas that are
          # good but not actionable right now.
          class Park < Dry::CLI::Command
            include Ace::Core::CLI::DryCli::Base

            desc <<~DESC.strip
              Park idea for later

              Moves the idea to the maybe/ subdirectory.
              Use for good ideas that aren't actionable right now.
              Restore with 'idea unpark'.

            DESC

            example [
              'future-feature        # Park idea by partial name',
