      ...
      module Idea
        ...
        class Create < Dry::CLI::Command
          ...
          def execute_create(capture_options)
            ...
            writer = Organisms::IdeaWriter.new(target_config)
            path = writer.write(capture_options[:content], capture_options)

            # Use project root, not .ace-taskflow root
            relative_path = Atoms::PathFormatter.format_relative_path(path, Dir.pwd)
          ...
          def determine_location(capture_options, config, root_path)
            ...
            Molecules::IdeaArgParser.determine_location(capture_options, config)
          end
