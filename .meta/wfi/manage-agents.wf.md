...
Create, update, and maintain agent definitions following the standardized agent definition guide. Ensure all agents follow single-purpose design, have proper response formats, and are correctly integrated with the project structure.

## Prerequisites

- Access to `dev-handbook/.integrations/claude/agents/` directory
...
- [ ] Determine if creating new agent or updating existing
- [ ] Identify agent's single purpose and action keywords
- [ ] Plan expected parameters and tool requirements
- [ ] Consider agent composition needs

### Execution Steps

- [ ] Create/update agent file with proper .ag.md extension
- [ ] Add standardized metadata and response format
- [ ] Update symlinks in .claude/agents/
- [ ] Update CLAUDE.md agent section if significant changes
- [ ] Update settings.json if new tool permissions needed
...
Create file in `dev-handbook/.integrations/claude/agents/[name].ag.md`:
...
```bash
cd .claude/agents
ln -s ../../dev-handbook/.integrations/claude/agents/[name].ag.md
```
...
<documents>
<document path="dev-handbook/.meta/gds/agents-definition.g.md">
...
ls dev-handbook/.integrations/claude/agents/*.ag.md

# Verify symlinks
ls -la .claude/agents/

# Check for tools field (should be minimal)
grep "^tools:" dev-handbook/.integrations/claude/agents/*.ag.md

# Verify response format sections
grep -l "## Response Format" dev-handbook/.integrations/claude/agents/*.ag.md
```
