...
module Ace
  module Hitl
    module Providers
      class Lab
        # Builds and runs the relay request invocation on behalf of the
        # provider=lab adapter (spec 8wm.t.vrz §1.1, §7). Sole owner of the
        # lab transport binary reference in ace-hitl code; the guard test
        # keeps every other agent-facing path free of it. The argv contract
        # is stable: existing required flags first, effect flags in a fixed
        # order (match, args, cwd, timeout). Values pass through verbatim;
        # no answer content or effect argv is ever logged here.
        class Transport
          ...
          def submit(argv)
            ...
            unless status.success?
              raise ProviderUnavailableError,
                "lab-hitl request failed (exit #{status.exitstatus}): #{stderr.to_s.strip}"
            end
          ...
          def run(argv)
            ...
            raise ProviderUnavailableError, "could not execute #{@bin}: #{e.class}: #{e.message}"
          end

          def parse_request_id(stdout)
            value = JSON.parse(stdout)
            id = value["id"]
            raise ProviderUnavailableError, "lab-hitl request returned no id" if id.nil? || id.to_s.strip.empty?

            id
          rescue JSON::ParserError => e
            raise ProviderUnavailableError, "could not parse lab-hitl request output as JSON: #{e.message}"
          end
        end
      end
    end
