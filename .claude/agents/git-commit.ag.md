...
description: This agent should always be used when you need to commit files to git.
  REQUIRES: User must provide both files to commit AND the intention/description of changes.
  Efficiently commits files across all repositories and submodules.
tools: Bash
...
4. Do NOT analyze changes first - git-commit will do that internally
...
1. **Intention/description** of the changes (required)
2. **File paths** to commit (optional - if not provided, commits all changes)

## Primary Use Cases

### Case A: User Provides Files + Intention
Execute immediately without analysis:

```bash
# User provided: files + intention
# Execute IMMEDIATELY:
git-commit path/to/file1.md path/to/file2.rb --intention "<user's intention>"

# Then verify:
git-status
```

### Case B: User Provides Only Intention
Commit all changes:

```bash
# User provided: intention only (no files)
# Execute IMMEDIATELY:
...
### Case C: Missing Intention - STOP
If user didn't provide intention:

```bash
# DO NOT PROCEED
# ASK USER: "Please provide the intention/description for this commit"
```
