...
- Scripts: `bin/test-review` (prompt generator), `exe/llm-query` (LLM interface)
...
- **System prompt**: `docs-dev/guides/code-review/_test-review-system.md`
...
mkdir -p docs-project/current/v.0.2.0-synapse/test_review/uncommitted-changes-$(date +%Y%m%d-%H%M%S)
cd docs-project/current/v.0.2.0-synapse/test_review/uncommitted-changes-*
...
exe/llm-query gpro tr-prompt.md \
  --system docs-dev/guides/code-review/_test-review-system.md \
  --timeout 300 \
...
mkdir -p docs-project/current/v.X.X.X-release/test_review/task-N/
cd docs-project/current/v.X.X.X-release/test_review/task-N/
```
...
for provider in gpro csonet; do
  exe/llm-query $provider tr-prompt.md \
    --system docs-dev/guides/code-review/_test-review-system.md \
    --timeout 300 \
...
mkdir -p docs-project/current/v.0.2.0-synapse/test_review/task-42/
cd docs-project/current/v.0.2.0-synapse/test_review/task-42/
git diff -- 'spec/**' ':!spec/cassettes/**' > input.diff
bin/test-review -d input.diff -o tr-prompt.md
exe/llm-query gpro tr-prompt.md --system docs-dev/guides/code-review/_test-review-system.md --timeout 300 -o tr-report-gpro.md
```
...
bin/test-review -d input.diff -o tr-prompt.md
exe/llm-query csonet tr-prompt.md --system docs-dev/guides/code-review/_test-review-system.md --timeout 300 -o tr-report-csonet.md
...
```
docs-project/current/v.X.X.X-release/test_review/task-N/
...
exe/llm-query gpro docs-dev/guides/code-review/_meta-test-review-combine.md \
  --input "tr-report-*.md" \
  --timeout 300 \
