...
- **Issue**: A critical inconsistency exists for core project documentation paths. The authoritative `architecture.md` file places them in `.ace/taskflow/`, but most workflows reference them in a non-existent `docs/` directory.
...
- **Required Fix**: All `Project Context Loading` sections must be updated to use the correct `.ace/taskflow/` prefix (e.g., `.ace/taskflow/what-do-we-build.md`).
...
- `create-adr.wf.md`:
  - **Issue**: Contains inconsistent path references for ADRs, mentioning `docs/decisions/` and `docs/architecture-decisions/`, while the project standard is `.ace/taskflow/decisions/`.
  - **Required Fix**: Standardize all paths to `.ace/taskflow/decisions/`.
...
- **Missing Guides – Required Workflow – File Path – Priority**
  ...
  - **File Path**: `.ace/handbook/guides/project-management/release-directory-contents.g.md`
  - **Priority**: 🟡 High
...
- **Missing Guides – Required Workflow – File Path – Priority**
  ...
  - **File Path**: `.ace/handbook/guides/project-tools/cli-reference.g.md` or update `architecture.md`.
  - **Priority**: 🟢 Medium
...
- **Path Ambiguity**: The primary issue is the `docs/` vs `.ace/taskflow/` path problem. An agent following `load-project-context.wf.md` will fail because the specified files do not exist at the given path. This is a workflow-breaking ambiguity.
...
- **Core Document Paths**: All 18 workflow files incorrectly reference core documentation (e.g., `what-do-we-build.md`) in `docs/` instead of the correct `.ace/taskflow/` directory. This is a critical, systemic error.
- **Internal Workflow Paths**: `create-adr.wf.md` contains conflicting internal paths, referencing both `docs/decisions/` and `docs/architecture-decisions/` when the correct path is `.ace/taskflow/decisions/`.
...
- [ ] **Fix Core Doc Paths**: Update all `Project Context Loading` sections in all 18 workflow files to reference `.ace/taskflow/what-do-we-build.md`, `.ace/taskflow/architecture.md`, and `.ace/taskflow/blueprint.md`.
...
- [ ] **Fix `create-adr.wf.md` Paths**: Standardize all internal directory references to `.ace/taskflow/decisions/`.
