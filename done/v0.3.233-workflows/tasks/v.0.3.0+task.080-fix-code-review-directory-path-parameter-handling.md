...
Fix the code-review command parameter handling so that directory paths (e.g., `dev-tools/lib`) and directory glob patterns (e.g., `dev-tools/lib/**/*`) work equivalently to explicit file glob patterns (e.g., `dev-tools/lib/**/*.rb`). Currently, directory paths fail with "undefined method 'find_files_with_pattern'" error in the FileSystemScanner class.
...
- `dev-tools/lib/coding_agent_tools/atoms/taskflow_management/file_system_scanner.rb` (implement missing method)
...
- [x] Locate the FileSystemScanner class and analyze its current implementation
  ...
  > Command: grep -r "class.*FileSystemScanner" dev-tools/lib/
...
- [x] Add logic to convert directory paths to appropriate glob patterns (e.g., `dir` → `dir/**/*`)
  ...
  > Command: code-review code dev-tools/lib --dry-run
...
- [x] AC 1: `code-review code dev-tools/lib` works without errors
- [x] AC 2: `code-review code dev-tools/lib/**/*` works equivalently to explicit patterns
- [x] AC 3: Both directory paths and glob patterns produce the same file selection results
- [x] AC 4: Existing glob pattern functionality (`dev-tools/lib/**/*.rb`) continues to work unchanged
...
michalczyz  …/handbook-meta   master ✘!⇡   v24.3.0
 ♥ 18:14 ➜ code-review code dev-tools/lib
...
```
michalczyz  …/handbook-meta   master ✘!⇡   v24.3.0
 ♥ 18:14 ➜ code-review code dev-tools/lib/**/*.rb
