...
| R‑TASK‑4 | All utilities shall source data via `.ace/tools/exe-old/*` scripts and return JSON when `--json` flag is passed.                                                          | P2       |
...
| R‑TCI‑3 | The LLM-generated output, formatted by the template, shall be saved as a new Markdown file in `.ace/taskflow/backlog/ideas/`.                                               | P1       |
| R‑TCI‑4 | A guide document (`.ace/handbook/guides/how-to-capture-ideas.guide.md`) shall be created to detail the usage of `bin/tci`.                                                      | P2       |
...
* `.ace/tools/exe-old/*` scripts present in codebase.
