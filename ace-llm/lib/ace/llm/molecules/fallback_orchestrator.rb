    ...
    module Molecules
      ...
      class FallbackOrchestrator
        ...
        def execute(primary_provider:, registry:)
          ...
          result = try_provider_with_retry(primary_provider, registry) { |client, provider_name| yield client, provider_name }
          ...
          @config.providers_for(primary_provider).each do |fallback_provider|
            ...
            result = try_provider_with_retry(fallback_provider, registry) { |client, provider_name| yield client, provider_name }
            return result if result
          end
