# ADR-011: ATOM Architecture House Rules for Component Classification
...
- Integration test strategy and VCR configuration (ADR-006)
- Zeitwerk autoloading configuration (ADR-007)
- Observability with dry-monitor (ADR-008)
- Centralized CLI error reporting (ADR-009)
- HTTP client strategy with Faraday (ADR-010)
- Future ADRs on component lifecycle management and dependency injection

## References

- [LlmModelInfo Refactoring Example](dev-taskflow/current/v.0.2.0-synapse/code-review/task-4/code-review-combined.md) - Primary example of Model vs Molecule classification
- [Architecture Documentation](docs/architecture.md) - ATOM structure definition and component examples
- [ATOM Component Classification House Rules](dev-handbook/guides/atom-house-rules.md) - Practical implementation guide for these rules
- [Code Review Guidelines](dev-handbook/guides/code-review/README.md) - Review process for architectural decisions
