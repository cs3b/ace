...
- **Model Discovery and Management**: List and filter available models from different providers, with the ability to select specific models for queries using the --model flag.
  - **Unified Model Discovery**: Discover available models via `llm-models <provider>` with caching support
  - **Multi-Provider Support**: Support for Google Gemini and LM Studio providers with consistent interface
