...
- **ace-llm v0.33.4**: Restored fallback parser compatibility coverage for model-qualified provider targeting by ensuring release-grade fallback paths preserve provider/model selectors.
- **ace-llm v0.33.5**: Added parser-aware provider fallback coverage for model-qualified selectors and stronger per-attempt option isolation during role fallback orchestration.
