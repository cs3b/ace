...
```bash
wc -l .ace/tools/exe-old/markdown-sync-embedded-documents | sed 's/^/    /'
```

_Result excerpt:_

```
    442 .ace/tools/exe-old/markdown-sync-embedded-documents
```
...
* .ace/tools/exe/handbook (executable)
...
* .ace/taskflow/current/v.0.3.0-migration/tasks/v.0.3.0+task.17-implement-sync-templates-command.md
...
* [x] Deep dive into 442-line synchronization logic and XML format analysis
  ...
  > Command: grep -E "templates|documents|guide" .ace/tools/exe-old/markdown-sync-embedded-documents | wc -l
* [x] Analyze existing CLI patterns (task-manager, code-review, nav-path)
  ...
  > Command: find .ace/tools/lib/coding_agent_tools/cli/commands -name "*.rb" | head -5
* [x] Review security integration patterns from FileIOHandler
  > TEST: Security Integration
  > Type: Research Check
  > Assert: Security validation patterns understood
  > Command: grep -n "SecurePathValidator\|FileOperationConfirmer" .ace/tools/lib/coding_agent_tools/molecules/file_io_handler.rb
...
- [x] Create xml_template_parser.rb molecule with comprehensive XML format support
  ...
  > Command: cd .ace/tools && bundle exec rspec spec/coding_agent_tools/molecules/task_management/xml_template_parser_spec.rb -e "format_support"
- [x] Create file_synchronizer.rb molecule with security integration
  ...
  > Command: cd .ace/tools && bundle exec rspec spec/coding_agent_tools/molecules/task_management/file_synchronizer_spec.rb -e "security"
- [x] Implement TemplateSynchronizer organism structure
  > TEST: Basic Structure
  > Type: Unit Test
  > Assert: Organism initializes correctly and coordinates molecules
  > Command: cd .ace/tools && bundle exec rspec spec/coding_agent_tools/organisms/task_management/template_synchronizer_spec.rb -e "initialize"
- [x] Port XML parsing logic for all formats (templates, documents, guide tags)
  ...
  > Command: cd .ace/tools && bundle exec rspec spec/coding_agent_tools/molecules/task_management/xml_template_parser_spec.rb -e "edge_cases"
...
- [x] Add dry-run mode with diff preview
  ...
  > Command: cd .ace/tools && bundle exec rspec spec/coding_agent_tools/organisms/task_management/template_synchronizer_spec.rb -e "dry_run"
...
- [x] Create handbook sync-templates CLI command using ExecutableWrapper pattern
  ...
  > Command: cd .ace/tools && bundle exec exe/coding_agent_tools handbook sync-templates --help
- [x] Create handbook executable using ExecutableWrapper pattern
  ...
  > Command: cd .ace/tools && exe/handbook sync-templates --help
...
* Original tool: .ace/tools/exe-old/markdown-sync-embedded-documents (442 lines)
