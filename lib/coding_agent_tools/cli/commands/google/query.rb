...
module CodingAgentTools
  module Cli
    module Commands
      module Google
        # Query command for interacting with Google API
        class Query < Dry::CLI::Command
          desc "Query Google AI with a prompt"
          ...
          def call(prompt:, **options)
            ...
            # Initialize and query Google
            response = query_google(prompt_text, system_text, options)
          ...
          def query_google(prompt_text, system_text, options)
            ...
            new_error = CodingAgentTools::Error.new("Failed to query Google: #{e.message}")
            new_error.set_backtrace(e.backtrace)
            raise new_error
          end

          def build_google_client(options)
            client_options = {}
