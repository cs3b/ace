...
module CodingAgentTools
  module Cli
    module Commands
      module Mistral
        # Query command for interacting with Mistral AI API
        class Query < Dry::CLI::Command
          desc "Query Mistral AI API with a prompt"
          ...
          option :model, type: :string, default: "mistral-large-latest",
            desc: "Model to use (default: mistral-large-latest)"

          option :temperature, type: :float,
            desc: "Temperature for generation (0.0-1.0)"
          ...
          def call(prompt:, **options)
            ...
            # Initialize and query Mistral
            response = query_mistral(prompt_text, system_text, options)
          ...
          def query_mistral(prompt_text, system_text, options)
            client = build_mistral_client(options)

            generation_options = build_generation_options(options, system_text)

            client.generate_text(prompt_text, **generation_options)
          rescue => e
            new_error = CodingAgentTools::Error.new("Failed to query Mistral AI: #{e.message}")
            new_error.set_backtrace(e.backtrace)
            raise new_error
          end
