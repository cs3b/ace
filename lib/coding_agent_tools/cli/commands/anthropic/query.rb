...
module CodingAgentTools
  module Cli
    module Commands
      module Anthropic
        # Query command for interacting with Anthropic API
        class Query < Dry::CLI::Command
          desc "Query Anthropic Claude API with a prompt"
          ...
          option :model, type: :string, default: "claude-3-5-sonnet-20241022",
            desc: "Model to use (default: claude-3-5-sonnet-20241022)"
          ...
          def call(prompt:, **options)
            ...
            # Initialize and query Anthropic
            response = query_anthropic(prompt_text, system_text, options)
          ...
          def query_anthropic(prompt_text, system_text, options)
            ...
            new_error = CodingAgentTools::Error.new("Failed to query Anthropic: #{e.message}")
            new_error.set_backtrace(e.backtrace)
            raise new_error
          end
