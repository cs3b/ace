    ...
    module CLI
      module DryCli
        ...
        module Base
          ...
          #
          # Uses strict conversion (Integer/Float) to catch invalid inputs rather
          # than silently converting "abc" to 0 (which to_i does).
          #
          # @param options [Hash] Command options hash
          # @param conversions [Hash] Map of option keys to conversion types (:integer, :float)
          # @return [Hash] Options with converted values
          # @raise [ArgumentError] if a value cannot be converted to the specified type
          #
