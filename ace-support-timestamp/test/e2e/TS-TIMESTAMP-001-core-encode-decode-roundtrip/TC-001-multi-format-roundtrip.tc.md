...
Verify that ace-timestamp CLI correctly encodes a timestamp to all 7 format precisions and decodes them back, producing IDs of the expected character lengths.
...
1. Encode a fixed timestamp to all 7 formats and verify ID lengths
   ...
   MONTH_ID=$(ace-timestamp encode --format month -q "$TS")
   WEEK_ID=$(ace-timestamp encode --format week -q "$TS")
   DAY_ID=$(ace-timestamp encode --format day -q "$TS")
   MIN40_ID=$(ace-timestamp encode --format 40min -q "$TS")
   SEC2_ID=$(ace-timestamp encode -q "$TS")
   MS50_ID=$(ace-timestamp encode --format 50ms -q "$TS")
   MS_ID=$(ace-timestamp encode --format ms -q "$TS")
...
3. Decode each format and verify date preservation
   ```bash
   DECODED_MONTH=$(ace-timestamp decode -q "$MONTH_ID")
   DECODED_DAY=$(ace-timestamp decode -q "$DAY_ID")
   DECODED_SEC2=$(ace-timestamp decode -q "$SEC2_ID")
   DECODED_MS=$(ace-timestamp decode -q "$MS_ID")
