...
1. Run config command and verify defaults
   ```bash
   CONFIG_OUTPUT=$(ace-timestamp config)
   echo "$CONFIG_OUTPUT"
...
2. Run verbose config and verify additional details
   ```bash
   VERBOSE_OUTPUT=$(ace-timestamp config --verbose)
   echo "$VERBOSE_OUTPUT"
...
3. Verify verbose has more output than basic
   ```bash
   BASIC_LINES=$(ace-timestamp config | wc -l)
   VERBOSE_LINES=$(ace-timestamp config --verbose | wc -l)
