...
Verify the full end-to-end pipeline: encode a timestamp via the ace-timestamp binary, decode the result, and confirm the decoded output matches the original input within precision bounds.
...
3. Verify roundtrip for month and ms formats
   ```bash
   MONTH_ENC=$(ace-timestamp encode --format month -q '2025-06-15 12:30:45 UTC')
   MONTH_DEC=$(ace-timestamp decode -q "$MONTH_ENC")
   echo "Month: $MONTH_ENC -> $MONTH_DEC"
   echo "$MONTH_DEC" | grep -q "2025-06" && echo "PASS: Month roundtrip" || echo "FAIL: Month roundtrip"

   MS_ENC=$(ace-timestamp encode --format ms -q '2025-06-15 12:30:45 UTC')
   MS_DEC=$(ace-timestamp decode -q "$MS_ENC")
