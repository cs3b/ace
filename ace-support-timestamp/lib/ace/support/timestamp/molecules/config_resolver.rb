    ...
    module Molecules
      ...
      module ConfigResolver
        ...
        class << self
          ...
          def load_config
            @config ||= begin
              gem_root = Gem.loaded_specs["ace-support-timestamp"]&.gem_dir ||
              ...
              warn "Error: Failed to parse #{config_path}: #{e.message}" if Ace::Support::Timestamp.debug?
              warn "  Check YAML syntax at line #{e.line}, column #{e.column}" if Ace::Support::Timestamp.debug? && e.line
              FALLBACK_DEFAULTS.dup
            rescue StandardError => e
              warn "Warning: Could not load ace-support-timestamp config: #{e.message}" if Ace::Support::Timestamp.debug?
              FALLBACK_DEFAULTS.dup
            end
          end
