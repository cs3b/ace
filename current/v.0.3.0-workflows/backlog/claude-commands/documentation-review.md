...
- Scripts: `bin/cr-docs` (prompt generator), `dev-tools/dev-tools/exe/llm-query` (LLM interface)
...
cd dev-taskflow/current/v.0.2.0-synapse/doc_review/uncommitted-changes-*
...
dev-tools/dev-tools/exe/llm-query gpro dr-prompt.md \
  --system dev-handbook/guides/code-review/_doc-review-system.md \
  --timeout 300 \
...
mkdir -p dev-taskflow/current/v.X.X.X-release/doc_review/task-N/
cd dev-taskflow/current/v.X.X.X-release/doc_review/task-N/
```
...
for provider in gpro csonet; do
  dev-tools/dev-tools/exe/llm-query $provider dr-prompt.md \
    --system dev-handbook/guides/code-review/_doc-review-system.md \
    --timeout 300 \
...
cd dev-taskflow/current/v.0.2.0-synapse/doc_review/task-42/
git diff -- 'lib/**' > input.diff
bin/cr-docs -d input.diff -o dr-prompt.md
dev-tools/dev-tools/exe/llm-query gpro dr-prompt.md --system dev-handbook/guides/code-review/_doc-review-system.md --timeout 300 -o dr-report-gpro.md
```
...
bin/cr-docs -d input.diff -o dr-prompt.md
dev-tools/dev-tools/exe/llm-query csonet dr-prompt.md --system dev-handbook/guides/code-review/_doc-review-system.md --timeout 300 -o dr-report-csonet.md
```
...
```
dev-taskflow/current/v.X.X.X-release/doc_review/task-N/
...
dev-tools/dev-tools/exe/llm-query gpro dev-handbook/guides/code-review/_meta-doc-review-combine.md \
  --input "dr-report-*.md" \
  --timeout 300 \
