...
| Candidate: analyze/analyze-consistency semantic matrix in E2E | MOVED | Covered by dedicated `TS-DOCS-002-analysis-workflows` so this smoke scenario remains focused on discover/validate/status/update core docs operations. | `test/fast/cli/commands/analyze_test.rb`, `test/fast/cli/commands/analyze_consistency_test.rb` |
