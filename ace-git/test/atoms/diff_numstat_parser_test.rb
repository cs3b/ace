...
class DiffNumstatParserTest < AceGitTestCase
  ...
  def test_parse_standard_numstat_lines
    output = "93\t25\tace-taskflow/lib/ace/taskflow/organisms/task_manager.rb\n1\t1\tace-taskflow/lib/ace/taskflow/version.rb"

    result = @parser.parse(output)

    assert_equal 2, result.length
    assert_equal "ace-taskflow/lib/ace/taskflow/organisms/task_manager.rb", result[0][:path]
  ...
  def test_parse_binary_line_sets_binary_flag
    output = "-\t-\tace-taskflow/test/fixtures/archive.bin"
