...
class ApiFetcherTest < ModelsDevTestCase
  def setup
  ...
  def teardown
    ...
    Ace::LLM::ModelsDev::Atoms::ApiFetcher.instance_variable_set(:@connection, nil)
  end

  def test_fetch_success
    stub_request_with_response(200, '{"test": "data"}')

    result = Ace::LLM::ModelsDev::Atoms::ApiFetcher.fetch("http://test.example/api.json")
  ...
  def test_fetch_raises_api_error_on_404
    ...
    error = assert_raises(Ace::LLM::ModelsDev::ApiError) do
      Ace::LLM::ModelsDev::Atoms::ApiFetcher.fetch("http://test.example/api.json")
    end
  ...
  def test_fetch_raises_network_error_on_timeout
    ...
    error = assert_raises(Ace::LLM::ModelsDev::NetworkError) do
      Ace::LLM::ModelsDev::Atoms::ApiFetcher.fetch("http://test.example/api.json")
    end
  ...
  def test_api_url_constant
    assert_equal "https://models.dev/api.json",
                 Ace::LLM::ModelsDev::Atoms::ApiFetcher::API_URL
  end

  def test_timeout_constant
    assert_equal 30, Ace::LLM::ModelsDev::Atoms::ApiFetcher::TIMEOUT
  end

  def test_max_retries_constant
    assert_equal 2, Ace::LLM::ModelsDev::Atoms::ApiFetcher::MAX_RETRIES
  end
  ...
  def stub_connection_singleton(connection)
    Ace::LLM::ModelsDev::Atoms::ApiFetcher.instance_variable_set(:@connection, connection)
  end
end
