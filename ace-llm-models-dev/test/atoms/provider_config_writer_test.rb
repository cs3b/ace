...
class ProviderConfigWriterTest < ModelsDevTestCase
  def setup
    @writer = Ace::LLM::ModelsDev::Atoms::ProviderConfigWriter
  end
  ...
  def test_update_models_raises_for_missing_file
    assert_raises(Ace::LLM::ModelsDev::ConfigError) do
  ...
  def test_update_models_raises_for_inline_comment
    with_temp_config_dir do |dir|
      ...
      error = assert_raises(Ace::LLM::ModelsDev::ConfigError) do
