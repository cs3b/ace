...
class ModelInfoTest < ModelsDevTestCase
  def setup
    @model_hash = sample_provider_data["models"]["test-model"]
  end

  def test_from_hash
    model = Ace::LLM::ModelsDev::Models::ModelInfo.from_hash(@model_hash, provider_id: "test-provider")
  ...
  def test_deprecated
    model = Ace::LLM::ModelsDev::Models::ModelInfo.new(status: "deprecated")
    assert model.deprecated?

    model2 = Ace::LLM::ModelsDev::Models::ModelInfo.new(status: nil)
    refute model2.deprecated?
  end

  def test_preview
    %w[alpha beta preview].each do |status|
      model = Ace::LLM::ModelsDev::Models::ModelInfo.new(status: status)
      assert model.preview?, "Expected #{status} to be preview"
    end

    model = Ace::LLM::ModelsDev::Models::ModelInfo.new(status: nil)
