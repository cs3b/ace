        ...
        class ProviderSyncOrchestrator
          ...
          def ensure_cache_fresh
            unless @cache_manager.exists?
              raise CacheError, "No models.dev cache found. Run 'ace-llm-models sync' first."
            end

            unless @cache_manager.fresh?(max_age: PROVIDER_SYNC_CACHE_MAX_AGE)
              output.puts "Warning: models.dev cache is more than 7 days old. Consider running 'ace-llm-models sync'."
            end
          end
