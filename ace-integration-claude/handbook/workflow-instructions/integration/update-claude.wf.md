...
Maintain Claude Code integration using unified handbook CLI commands to ensure all workflows have corresponding Claude
commands and the integration remains synchronized with the latest handbook updates.
...
* Scans all command files in dev-tools/
...
* Check file permissions in dev-tools/
...
* Tools must be simple names: `Bash`, `Read`, `Write`, `Edit`, `Grep`, `Glob`, `LS`, `TodoWrite`, `WebSearch`,
  `WebFetch`
...
### Diagnostic Commands
    ...
    grep -r "Bash(" dev-tools/
    
    # List meta workflow commands
    ls -la dev-tools/commands/_custom/meta-*.md
{: .language-bash}
...
*This meta workflow ensures your Claude Code integration remains synchronized with the handbook's workflow instructions.
Run it regularly to maintain optimal AI-assisted development experience.*
