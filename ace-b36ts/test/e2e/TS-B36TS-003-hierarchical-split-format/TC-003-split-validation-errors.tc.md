...
1. Verify invalid split order is rejected (day,month)
   ```bash
   ace-b36ts encode --split day,month '2025-06-15 14:32:45' 2>&1
   INVALID_ORDER=$?
...
2. Verify block without day is rejected
   ```bash
   ace-b36ts encode --split month,block '2025-06-15 14:32:45' 2>&1
   MISSING_DEP=$?
...
3. Verify --split and --format mutual exclusivity
   ```bash
   ace-b36ts encode --split month --format day '2025-06-15 14:32:45' 2>&1
   MUTUAL=$?
...
4. Verify unknown split level is rejected
   ```bash
   ace-b36ts encode --split month,invalid '2025-06-15 14:32:45' 2>&1
   UNKNOWN=$?
...
5. Verify split must start with month
   ```bash
   ace-b36ts encode --split week,day '2025-06-15 14:32:45' 2>&1
   NO_MONTH=$?
