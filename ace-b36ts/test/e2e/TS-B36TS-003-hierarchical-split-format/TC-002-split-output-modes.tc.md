...
1. Verify path-only is a single usable line
   ...
   PATH_ONLY=$(ace-b36ts encode --split month,week --path-only -q "$TS")
   echo "Path: $PATH_ONLY"
...
2. Verify JSON output structure
   ```bash
   JSON_OUT=$(ace-b36ts encode --split month,week,day --json -q "$TS")
   echo "$JSON_OUT"
...
3. Verify decode from path with different separators
   ```bash
   SLASH_PATH=$(ace-b36ts encode --split month,week,day --path-only -q "$TS")
   COLON_PATH=$(echo "$SLASH_PATH" | tr '/' ':')

   DECODED_SLASH=$(ace-b36ts decode -q "$SLASH_PATH")
   DECODED_COLON=$(ace-b36ts decode -q "$COLON_PATH")
