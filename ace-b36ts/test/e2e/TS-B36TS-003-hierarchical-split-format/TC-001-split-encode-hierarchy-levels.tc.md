...
1. Encode with month-only split
   ...
   MONTH_PATH=$(ace-b36ts encode --split month --path-only -q "$TS")
...
2. Encode with month,week and month,day splits
   ```bash
   MW_PATH=$(ace-b36ts encode --split month,week --path-only -q "$TS")
   MD_PATH=$(ace-b36ts encode --split month,day --path-only -q "$TS")
...
3. Encode with full hierarchy month,week,day,block
   ```bash
   FULL_PATH=$(ace-b36ts encode --split month,week,day,block --path-only -q "$TS")
...
4. Verify split roundtrip preserves timestamp
   ```bash
   ORIGINAL_ID=$(ace-b36ts encode -q "$TS")
   SPLIT_PATH=$(ace-b36ts encode --split month,week,day --path-only -q "$TS")
   DECODED_SPLIT=$(ace-b36ts decode -q "$SPLIT_PATH")
   DECODED_ORIG=$(ace-b36ts decode -q "$ORIGINAL_ID")
