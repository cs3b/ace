...
1. Encode multiple timestamps in chronological order
   ```bash
   ID1=$(ace-b36ts encode -q '2025-01-01 00:00:00')
   ID2=$(ace-b36ts encode -q '2025-03-15 06:30:00')
   ID3=$(ace-b36ts encode -q '2025-06-15 12:00:00')
   ID4=$(ace-b36ts encode -q '2025-12-31 23:59:59')
...
3. Verify day/week disambiguation (3-char IDs sort correctly)
   ```bash
   WEEK_ID=$(ace-b36ts encode --format week -q '2025-06-18 14:32:45')
   DAY_ID=$(ace-b36ts encode --format day -q '2025-06-18 14:32:45')
   WEEK_DECODED=$(ace-b36ts decode -q "$WEEK_ID")
   DAY_DECODED=$(ace-b36ts decode -q "$DAY_ID")
