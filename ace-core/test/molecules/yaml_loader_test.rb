...
class YamlLoaderTest < AceTestCase
  def test_loads_valid_yaml
    with_temp_dir do
      ...
      config = Ace::Core::Molecules::YamlLoader.load_file("test.yml")
  ...
  def test_handles_missing_file
    assert_raises(Ace::Core::ConfigNotFoundError) do
      Ace::Core::Molecules::YamlLoader.load_file("nonexistent.yml")
    end
  end

  def test_handles_missing_file_safe
    config = Ace::Core::Molecules::YamlLoader.load_file_safe("nonexistent.yml")
  ...
  def test_saves_config_to_file
    with_temp_dir do
      config = Ace::Core::Models::Config.new(
        {
          "ace" => {
            "saved" => true,
            "version" => "1.0.0"
      ...
      Ace::Core::Molecules::YamlLoader.save_file(config, "output.yml")

      assert File.exist?("output.yml")

      loaded = Ace::Core::Molecules::YamlLoader.load_file("output.yml")
  ...
  def test_saves_hash_to_file
    with_temp_dir do
      ...
      Ace::Core::Molecules::YamlLoader.save_file(data, "hash_output.yml")

      assert File.exist?("hash_output.yml")

      loaded = Ace::Core::Molecules::YamlLoader.load_file("hash_output.yml")
  ...
  def test_creates_directory_when_saving
    with_temp_dir do
      config = Ace::Core::Models::Config.new({ "test" => "data" })

      Ace::Core::Molecules::YamlLoader.save_file(config, "deep/nested/path/file.yml")
  ...
  def test_load_and_merge_multiple_files
    with_temp_dir do
      ...
      merged = Ace::Core::Molecules::YamlLoader.load_and_merge(
        "config1.yml",
        "config2.yml"
      )
  ...
  def test_handles_invalid_yaml_gracefully
    with_temp_dir do
      ...
      assert_raises(Ace::Core::YamlParseError) do
        Ace::Core::Molecules::YamlLoader.load_file("invalid.yml")
      end
    end
  end
  ...
  def test_handles_io_errors_on_save
    with_temp_dir do
      ...
      assert_raises(IOError) do
        Ace::Core::Molecules::YamlLoader.save_file(config, "readonly/subdir/file.yml")
      end
    ensure
