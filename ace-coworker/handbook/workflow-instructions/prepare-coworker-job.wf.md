...
Transform informal instructions OR preset names into a structured job.yaml file that can be used with `ace-coworker create job.yaml`. This workflow bridges the gap between high-level intent and the structured work queue format.
...
Start session with: ace-coworker create job.yaml
```
...
```bash
# Start the session
/ace:coworker-create-session job.yaml
