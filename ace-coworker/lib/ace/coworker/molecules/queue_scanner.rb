    ...
    module Molecules
      ...
      class QueueScanner
        ...
        def scan(jobs_dir, session:)
          ...
          # Get all step files
          files = Dir.glob(File.join(jobs_dir, "*.j.md"))
        ...
        # @param job_file_path [String] Path to job file
        # @param number [String] Step number
        # @param name [String] Step name
        # @return [String, nil] Report content or nil
        def load_report(job_file_path, number, name)
          # reports/ is sibling of jobs/
          jobs_dir = File.dirname(job_file_path)
