...
module Ace
  module Coworker
    module Molecules
      # Handles job file renumbering with cascade support and atomic operations.
      #
      # Extracted from WorkflowExecutor to provide:
      # - Testable renumbering logic
      # - Transactional rename with rollback on failure
      # - Parent metadata updates for descendants
      #
      # @example Basic usage
      #   renumberer = JobRenumberer.new(step_writer: step_writer, queue_scanner: scanner)
      #   renumberer.renumber(jobs_dir, ["010", "011"])
      class JobRenumberer
        ...
        # Renumber jobs by shifting their file numbers.
        # Also cascades to all descendants to prevent orphaning children.
        #
        # @param jobs_dir [String] Path to jobs directory
        # @param numbers_to_shift [Array<String>] Job numbers to shift
        # @return [Hash] Result with :renamed (count) and :rollback_needed (bool)
        def renumber(jobs_dir, numbers_to_shift)
          return { renamed: 0, rollback_needed: false } if numbers_to_shift.empty?

          all_numbers = queue_scanner.step_numbers(jobs_dir)
          sorted_jobs = build_shift_list(numbers_to_shift, all_numbers)
          ...
          begin
            sorted_jobs.each do |old_number|
              ...
              rename_result = rename_job_files(
                jobs_dir,
                old_number,
                new_number,
        ...
        # Build complete list of jobs to shift including descendants.
        #
        # @param numbers_to_shift [Array<String>] Explicit jobs to shift
        # @param all_numbers [Array<String>] All existing job numbers
        ...
        def build_shift_list(numbers_to_shift, all_numbers)
          ...
          numbers_to_shift.each do |num|
            all_to_shift << num
            # Find all descendants (jobs starting with "num.")
            all_numbers.each do |n|
              all_to_shift << n if Atoms::JobNumbering.child_of?(n, num)
            end
          end
          all_to_shift.uniq!
        ...
        # Calculate the new number for a job being shifted.
        #
        # @param old_number [String] Current job number
        # @param numbers_to_shift [Array<String>] Jobs explicitly being shifted
        # @return [String] New job number
        def calculate_new_number(old_number, numbers_to_shift)
          if numbers_to_shift.include?(old_number)
            Atoms::JobNumbering.shift_number(old_number, 1)
          else
            # This is a descendant - cascade parent shift
            parent_old = Atoms::JobNumbering.parse(old_number)[:parent]
            parent_new = if parent_old && numbers_to_shift.include?(parent_old)
                           Atoms::JobNumbering.shift_number(parent_old, 1)
                         end
            ...
            if parent_new
              ...
              ancestor = numbers_to_shift.find { |n| Atoms::JobNumbering.child_of?(old_number, n) }
              if ancestor
                new_ancestor = Atoms::JobNumbering.shift_number(ancestor, 1)
        ...
        #
        # @param old_number [String] Job being renamed
        # @param numbers_to_shift [Array<String>] Jobs explicitly being shifted
        ...
        def calculate_new_parent(old_number, numbers_to_shift)
          old_parent = Atoms::JobNumbering.parse(old_number)[:parent]
          return nil unless old_parent

          ancestor = numbers_to_shift.find { |n| old_parent == n || Atoms::JobNumbering.child_of?(old_parent, n) }
          if ancestor
            new_ancestor = Atoms::JobNumbering.shift_number(ancestor, 1)
        ...
        # Rename job and report files from one number to another.
        #
        # @param jobs_dir [String] Path to jobs directory
        # @param old_number [String] Old job number
        # @param new_number [String] New job number
        # @param new_parent [String, nil] New parent number for frontmatter update
        # @return [Hash] Result with :files (renamed file paths)
        def rename_job_files(jobs_dir, old_number, new_number, new_parent: nil)
          renamed_files = []

          # Find job file with this number
          pattern = File.join(jobs_dir, "#{old_number}-*.j.md")
          job_files = Dir.glob(pattern)

          job_files.each do |old_path|
            filename = File.basename(old_path)
            new_filename = filename.sub(/^#{Regexp.escape(old_number)}/, new_number)
            new_path = File.join(jobs_dir, new_filename)

            FileUtils.mv(old_path, new_path)
            renamed_files << { old_path: old_path, new_path: new_path, type: :job }
          ...
          cache_dir = File.dirname(jobs_dir)
        ...
        def rollback_renames(completed_renames)
          ...
          if rollback_errors.any?
            warn "[ace-coworker] Warning: #{rollback_errors.size} file(s) failed to rollback during renumber recovery:"
