...
class QueueScannerTest < AceCoworkerTestCase
  ...
  def test_scan_with_steps
    with_temp_cache do |cache_dir|
      ...
      # Create step files
      File.write(File.join(jobs_dir, "010-init.j.md"), <<~MD)
        ---
        name: init
        status: done
        ---
      ...
      File.write(File.join(jobs_dir, "020-build.j.md"), <<~MD)
        ---
        name: build
        status: in_progress
        ---
  ...
  def test_scan_sorts_correctly
    with_temp_cache do |cache_dir|
      ...
      File.write(File.join(jobs_dir, "030-third.j.md"), "---\nname: third\nstatus: pending\n---\nThird")
      File.write(File.join(jobs_dir, "010-first.j.md"), "---\nname: first\nstatus: done\n---\nFirst")
      File.write(File.join(jobs_dir, "020-second.j.md"), "---\nname: second\nstatus: done\n---\nSecond")
  ...
  def test_step_numbers
    with_temp_cache do |cache_dir|
      ...
      File.write(File.join(jobs_dir, "010-init.j.md"), "---\nname: init\nstatus: done\n---\nInit")
      File.write(File.join(jobs_dir, "020-build.j.md"), "---\nname: build\nstatus: pending\n---\nBuild")
