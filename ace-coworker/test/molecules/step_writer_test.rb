...
class StepWriterTest < AceCoworkerTestCase
  def test_create_step
    with_temp_cache do |cache_dir|
      ...
      assert_equal "010-init-project.j.md", File.basename(file_path)
  ...
  def test_mark_done_with_report
    with_temp_cache do |cache_dir|
      ...
      # Job file should have status done and completed_at, but no report content embedded
      job_content = File.read(file_path)
      assert_includes job_content, "status: done"
      assert_includes job_content, "completed_at:"
      refute_includes job_content, "All done!"
      refute_includes job_content, "# Report"
