...
class StartCommandTest < AceCoworkerTestCase
  def test_start_with_valid_config
    with_temp_cache do |cache_dir|
      ...
      output = capture_io do
        result = Ace::Coworker::CLI::Commands::Start.new.call(config: config_path)
      end
  ...
  def test_start_with_missing_config
    result = nil
    output = capture_io do
      result = Ace::Coworker::CLI::Commands::Start.new.call(config: "nonexistent.yaml")
    end
  ...
  def test_start_quiet_mode
    with_temp_cache do |cache_dir|
      ...
      output = capture_io do
        Ace::Coworker::CLI::Commands::Start.new.call(config: config_path, quiet: true)
      end
