...
class PresetExpanderTest < AceCoworkerTestCase
  # parse_array_parameter tests

  def test_parse_array_parameter_comma_separated
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("148,149,150")
  ...
  def test_parse_array_parameter_range
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("148-152")

    assert_equal ["148", "149", "150", "151", "152"], result
  end

  def test_parse_array_parameter_single_value
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("148")

    assert_equal ["148"], result
  end

  def test_parse_array_parameter_pattern_with_asterisk
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("240.*")

    assert_equal ["240.*"], result
  end

  def test_parse_array_parameter_pattern_with_question_mark
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("24?")

    assert_equal ["24?"], result
  end

  def test_parse_array_parameter_already_array
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter(["148", "149"])
  ...
  def test_parse_array_parameter_nil
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter(nil)

    assert_equal [], result
  end

  def test_parse_array_parameter_empty_string
    result = Ace::Coworker::Atoms::PresetExpander.parse_array_parameter("")
  ...
  def test_has_expansion_true
    ...
    assert Ace::Coworker::Atoms::PresetExpander.has_expansion?(preset)
  end

  def test_has_expansion_false
    preset = { "steps" => [] }

    refute Ace::Coworker::Atoms::PresetExpander.has_expansion?(preset)
  end
  ...
  def test_foreach_parameter_present
    ...
    assert_equal "taskrefs", Ace::Coworker::Atoms::PresetExpander.foreach_parameter(preset)
  end

  def test_foreach_parameter_absent
    preset = { "steps" => [] }

    assert_nil Ace::Coworker::Atoms::PresetExpander.foreach_parameter(preset)
  end
  ...
  def test_validate_parameters_all_present
    ...
    errors = Ace::Coworker::Atoms::PresetExpander.validate_parameters(preset, params)
  ...
  def test_expand_without_expansion_section
    ...
    result = Ace::Coworker::Atoms::PresetExpander.expand(preset, params)
  ...
  def test_expand_without_expansion_preserves_other_fields
    preset = {
      "steps" => [
        { "name" => "step1", "skill" => "ace:commit", "instructions" => "Commit" }
      ]
    }

    result = Ace::Coworker::Atoms::PresetExpander.expand(preset, {})

    assert_equal "ace:commit", result[0]["skill"]
  end
