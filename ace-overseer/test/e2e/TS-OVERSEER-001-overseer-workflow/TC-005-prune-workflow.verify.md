...
3. **Correct prune commands used** — command files show normal prune flow (`ace-overseer prune --dry-run`, `ace-overseer prune --yes`) and no assignment-prune flags.
...
- **FAIL**: Wrong prune mode/context, assignment completion missing, final worktree state incorrect, or captures missing.
