...
class Ace::Lint::Atoms::StandardrbRunnerTest < Minitest::Test
  ...
  def test_available_returns_true_when_standardrb_exists
    stub_standardrb_run(exit_status: 0) do
  ...
  def test_run_with_empty_output_returns_success
    stub_standardrb_run(output: "", exit_status: 0) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("test.rb")
  ...
  def test_run_with_no_offenses_returns_success
    stub_standardrb_run(output: "no offenses detected", exit_status: 0) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("test.rb")
      assert result[:success]
      assert_empty result[:errors]
      assert_empty result[:warnings]
  ...
  def test_run_with_rubocop_json_format_parses_correctly
    ...
    stub_standardrb_run(output: json_output, exit_status: 1) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("test.rb")
  ...
  def test_run_with_legacy_array_format_fallback
    ...
    stub_standardrb_run(output: json_output, exit_status: 1) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("legacy.rb")
  ...
  def test_run_with_text_output_fallback
    ...
    stub_standardrb_run(output: "", stderr: text_output, exit_status: 1) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("legacy.rb")
  ...
  def test_run_handles_json_parse_error
    stub_standardrb_run(output: "invalid { json", exit_status: 0) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("test.rb")
  ...
  def test_build_command_includes_fix_flag
    cmd = Ace::Lint::Atoms::StandardrbRunner.build_command(["test.rb"], fix: true)
    assert_includes cmd, "--fix"
    assert_includes cmd, "test.rb"
  end

  def test_build_command_without_fix
    cmd = Ace::Lint::Atoms::StandardrbRunner.build_command(["test.rb"], fix: false)
    refute_includes cmd, "--fix"
    assert_includes cmd, "test.rb"
  end

  def test_build_command_with_multiple_files
    cmd = Ace::Lint::Atoms::StandardrbRunner.build_command(["test1.rb", "test2.rb"], fix: false)
    assert_includes cmd, "test1.rb"
    assert_includes cmd, "test2.rb"
    assert_equal 5, cmd.size # ["standardrb", "--format", "json", "test1.rb", "test2.rb"]
  end

  def test_run_with_warnings_only_returns_failure_when_non_zero_exit
    # StandardRB exits non-zero for convention/warning level offenses
    ...
    stub_standardrb_run(output: json_output, exit_status: 1) do
      ...
      refute result[:success], "Expected success to be false when StandardRB exits non-zero with warnings"
  ...
  def test_run_with_warnings_only_returns_success_when_zero_exit
    # If StandardRB exits zero, warnings should not cause failure
    json_output = {
      "files" => [
        {
          "path" => "lib/test.rb",
    ...
    stub_standardrb_run(output: json_output, exit_status: 0) do
      result = Ace::Lint::Atoms::StandardrbRunner.run("test.rb")

      # With zero exit, success depends on error presence
      assert result[:success], "Expected success to be true when StandardRB exits zero"
      assert_equal 1, result[:warnings].size
    end
  end
  ...
  def test_text_output_with_warnings_only_returns_failure_when_non_zero_exit
    ...
    stub_standardrb_run(output: "", stderr: text_output, exit_status: 1) do
      ...
      # Critical: success must be false when exit status is non-zero
      refute result[:success], "Expected success to be false when StandardRB exits non-zero with text warnings"
      assert_equal 1, result[:warnings].size
      assert_empty result[:errors]
    end
  end
