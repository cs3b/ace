...
4. **No-report suppression** — The `--no-report` test shows exit code `0`, no `Reports:` line in output, and no new lint cache entries created for that run. Use the before/after snapshot artifacts for that sub-run; accepted names are either `cache.before` / `cache.after` / `cache.diff` or `no-report.before` / `no-report.after` / `no-report.diff`. An empty diff is valid proof even if `.ace-local/lint` already existed before the test.
