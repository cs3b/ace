...
class PlainMarkdownLoadTest < AceTestCase
  ...
  def test_loads_workflow_file_via_wfi_protocol
    ...
    # For isolated testing of ace-context without ace-nav, see test_loads_wfi_content_directly.
    unless $?.success? && File.exist?(file_path)
      skip "ace-nav CLI not available - E2E test requires ace-nav for wfi:// protocol resolution"
    end

    # Load via ace-context API (load_auto handles protocol resolution)
    result = Ace::Context.load_auto("wfi://commit")
  ...
  def test_loads_wfi_content_directly
    # Test ace-context can load workflow content without ace-nav dependency
    ...
    content = <<~MARKDOWN
      ...
      This is a test workflow file for testing ace-context without ace-nav.
    MARKDOWN

    File.write(temp_workflow, content)

    # Load the file directly via ace-context
    result = Ace::Context.load_file(temp_workflow)
  ...
  def test_loads_plain_markdown_file_by_path
    ...
    result = Ace::Context.load_file(temp_file)
